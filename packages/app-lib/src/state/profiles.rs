...
use crate::state::{CacheBehaviour, CachedEntry, CachedFileHash, cache_file_hash};
use crate::util;
...
impl Profile {
	...
	pub async fn remove(profile_path: &str, pool: &SqlitePool) -> crate::Result<()> {
		sqlx::query!(
			"
	...
	async fn parse_open_log_file(
		...
		static LOG_LINE_REGEX: LazyLock<Regex> = LazyLock::new(|| {
			Regex::new(
				r"^\[[0-9]{2}(?::[0-9]{2}){2}] \[.+?/[A-Z]+?]: Connecting to (.+?), ([1-9][0-9]{0,4})$",
			)
			.unwrap()
		});
	...
	#[tracing::instrument]
	pub async fn remove_project(profile_path: &str, project_path: &str) -> crate::Result<()> {
		if let Ok(path) = crate::api::profile::get_full_path(profile_path).await {
