...
impl ProcessManager {
	...
	pub async fn insert_new_process(
		...
		post_process_init: impl AsyncFnOnce(&ProcessMetadata, &RpcServer) -> crate::Result<()>,
		...
		if let Err(e) = post_process_init(&process.metadata, &process.rpc_server).await {
...
impl Process {
		...
		if let Some(timestamp_str) = timestamp {
			if let Ok(timestamp_val) = timestamp_str.parse::<i64>() {
				let datetime_utc = if timestamp_val > i32::MAX as i64 {
					...
					chrono::DateTime::<Utc>::from_timestamp(timestamp_val, 0).unwrap_or_default()
				};
	...
	async fn maybe_handle_server_join_logging(
		...
		let timestamp = timestamp
			.parse::<i64>()
			.map(|x| x / 1000)
			.map_err(|x| crate::ErrorKind::OtherError(format!("Failed to parse timestamp: {x}")))
			.and_then(|x| {
