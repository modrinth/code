...
impl DirectoryInfo {
	...
	pub fn get_initial_settings_dir() -> Option<PathBuf> {
		Self::env_path("THESEUS_CONFIG_DIR").or_else(|| Some(dirs::data_dir()?.join("ModrinthApp")))
	}
	...
	pub async fn init(config_dir: Option<String>) -> crate::Result<Self> {
		...
		fs::create_dir_all(&settings_dir).await.map_err(|err| {
			crate::ErrorKind::FSError(format!("Error creating Theseus config directory: {err}"))
		})?;
	...
	pub fn launcher_logs_dir() -> Option<PathBuf> {
		Self::get_initial_settings_dir().map(|d| d.join(LAUNCHER_LOGS_FOLDER_NAME))
	}
		...
		E: sqlx::Executor<'a, Database = sqlx::Sqlite> + Copy,
	{
		let app_dir = DirectoryInfo::get_initial_settings_dir().ok_or(
			crate::ErrorKind::FSError("Could not find valid config dir".to_string()),
		)?;
		...
		if let Some(ref prev_custom_dir) = settings.prev_custom_dir {
			...
			async fn is_dir_writeable(new_config_dir: &Path) -> crate::Result<bool> {
			...
			if prev_dir != move_dir {
				...
				async fn add_paths(
					...
					for entry_path in crate::pack::import::get_all_subfiles(source, false).await? {
				...
				if crate::util::io::is_same_disk(&prev_dir, &move_dir).unwrap_or(false) {
