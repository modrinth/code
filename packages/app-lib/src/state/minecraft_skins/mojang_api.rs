...
use crate::{
    data::Credentials,
    state::{MinecraftProfile, ProfileCacheEntry, PROFILE_CACHE},
...
async fn update_profile_cache_from_response(response: reqwest::Response) {
    let Some(mut profile) = response.json::<MinecraftProfile>().await.ok()
        ...
        tracing::warn!("Failed to parse player profile from skin or cape operation response, not updating profile cache");
        return;
    };
