...
use crate::state::{DirectoryInfo, InstanceLaunchContext, JavaVersion, ModLoader};
use crate::State;
...
impl RuntimeStorage {
	pub async fn read(state: &State) -> crate::Result<Self> {
		...
		for instance in instance_rows::list_instances(&state.pool).await? {
			match instance_rows::get_instance_launch_context(&instance.id, &state.pool).await {
		...
		let java = JavaVersion::get_all(&state.pool).await?
			.into_iter().collect::<HashMap<_, _>>();
		let mut dirs = state.directories.clone();
		dirs.config_dir = tokio::fs::canonicalize(&dirs.config_dir).await?;
		tokio::task::spawn_blocking(move || Self::scan(dirs, contexts, java, incomplete)).await?
	}
	...
	fn scan(
		...
		match fs::symlink_metadata(&root) {
			...
			Ok(_) => return Err(input("Runtime storage must be a directory, not a symlink")),
			Err(error) if error.kind() == std::io::ErrorKind::NotFound => return Ok(Self { root, files: Vec::new() }),
		...
		let mut pending = vec![
			dirs.versions_dir(), dirs.assets_dir(), dirs.libraries_dir(),
			dirs.java_versions_dir(), dirs.natives_dir(), dirs.log_configs_dir(),
		...
		while let Some(path) = pending.pop() {
			...
			if metadata.is_dir() {
				...
				let last_used_at = metadata.accessed().ok().into_iter()
					.chain(metadata.modified().ok())
					.filter_map(|time| time.duration_since(UNIX_EPOCH).ok())
					.map(|time| time.as_secs() as i64).max().unwrap_or(i64::MAX);
		...
		for file in files {
			let distribution = file.path.strip_prefix(dirs.java_versions_dir()).ok()
			...
			if let Some(path) = distribution {
				let entry = distributions.entry(path.clone()).or_insert(RuntimeFile {
					path, size: 0, last_used_at: 0, references: 0, directory: true,
				});
		...
		Ok(Self { root, files: individual_files })
	}
	...
	pub fn shared_bytes(&self) -> u64 {
		self.files.iter().filter(|file| file.references > 1).map(|file| file.size).sum()
	}

	pub fn unused_bytes(&self) -> u64 {
		self.files.iter().filter(|file| file.references == 0).map(|file| file.size).sum()
	}

	pub fn saved_bytes(&self) -> u64 {
		self.files.iter().map(|file| file.size.saturating_mul(file.references.saturating_sub(1) as u64)).sum()
	}
}
...
impl References {
	fn file(&mut self, path: PathBuf, owner: &str) {
		self.files.entry(normalize(&path)).or_default().insert(owner.to_owned());
	}

	fn tree(&mut self, path: &Path, owner: &str) {
		self.trees.entry(normalize(path)).or_default().insert(owner.to_owned());
	}
	...
	fn instance(
		...
		let version_id = if content.loader == ModLoader::Vanilla {
			...
			let loader = content.loader_version.as_deref()
				.filter(|version| !matches!(*version, "latest" | "stable"))
				.ok_or_else(|| input("The installed loader version is unresolved"))?;
			format!("{}-{loader}", content.game_version)
		};
		let version_dir = dirs.version_dir(&version_id);
		let version: VersionInfo = read_json(&version_dir.join(format!("{version_id}.json")))?;
		self.tree(&version_dir, owner);
		self.tree(&dirs.version_natives_dir(&version_id), owner);
		for library in &version.libraries {
			self.file(dirs.libraries_dir().join(daedalus::get_path_from_artifact(&library.name)?), owner);
		}
		if version.processors.as_ref().is_some_and(|processors| !processors.is_empty()) {
		...
		let index_path = dirs.assets_index_dir().join(format!("{}.json", version.asset_index.id));
		let index: AssetsIndex = read_json(&index_path)?;
		self.file(index_path, owner);
		for (name, asset) in index.objects {
			if asset.hash.len() != 40 || !asset.hash.bytes().all(|byte| byte.is_ascii_hexdigit()) {
		...
		let major = version.java_version.as_ref().map_or(8, |java| java.major_version);
		let selected = context.launch_overrides.java_path.as_deref()
			.filter(|path| Path::new(path).is_file())
			.into_iter()
			.chain(java.get(&major).map(|java| java.path.as_str()));
		for selected in selected {
			let selected = fs::canonicalize(selected)?;
			if let Ok(relative) = selected.strip_prefix(dirs.java_versions_dir())
...
fn read_json<T: DeserializeOwned>(path: &Path) -> crate::Result<T> {
	Ok(serde_json::from_reader(std::io::BufReader::new(fs::File::open(path)?))?)
}

pub(super) async fn remove_file(file: &RuntimeFile, root: &Path) -> crate::Result<u64> {
	let path = &file.path;
	let mut parent = path.parent();
	while let Some(directory) = parent {
		if !directory.starts_with(root) || tokio::fs::symlink_metadata(directory).await?.is_symlink() {
			return Err(input("Runtime cache path is outside the managed storage directory"));
		}
	...
	if metadata.is_dir() != file.directory { return Err(input("Runtime cache entry changed during cleanup")); }
	if file.directory {
	...
	let mut parent = path.parent();
	while let Some(directory) = parent {
		if directory == root || tokio::fs::remove_dir(directory).await.is_err() { break; }
		parent = directory.parent();
	}
	Ok(if metadata.is_file() { metadata.len() } else { 0 })
}
