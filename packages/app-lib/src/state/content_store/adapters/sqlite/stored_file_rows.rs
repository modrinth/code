...
pub(in crate::state::content_store) async fn stored_files(
    ...
    Ok(sqlx::query_as!(
		StoredFileMetadata,
		r#"SELECT sha512, sha1, size, relative_path, status AS "status: _", modified_at_ns, last_used_at, sources FROM store_blobs"#
...
pub(crate) async fn find_files(
    ...
    Ok(sqlx::query_as!(
		StoredFileMetadata,
		r#"SELECT sha512, sha1, size, relative_path, status AS "status: _", modified_at_ns, last_used_at, sources FROM store_blobs WHERE (? IS NOT NULL AND sha512 = ?) OR (? IS NULL AND sha1 = ?)"#,
		sha512, sha512, sha512, sha1
	).fetch_all(pool).await?)
}

pub(in crate::state::content_store) async fn find_file(
    pool: &SqlitePool,
    sha512: &str,
) -> crate::Result<Option<StoredFileMetadata>> {
    Ok(sqlx::query_as!(
		StoredFileMetadata,
		r#"SELECT sha512, sha1, size, relative_path, status AS "status: _", modified_at_ns, last_used_at, sources FROM store_blobs WHERE sha512 = ?"#,
		sha512
