...
use crate::state::{DirectoryInfo, ProfileInstallStage, ProjectType, attached_world_data};
		...
		for sub_path in ProjectType::iterator().map(|x| x.get_folder()).chain([
			...
			if !full_path.exists() && !full_path.is_symlink() {
				if !sub_path.contains(".") {
					...
					if let Err(e) = crate::util::io::write(&full_path, EMPTY_NBT).await {
						tracing::error!("Failed to create file for watcher {full_path:?}: {e}");
						return;
					}
				}
			}
