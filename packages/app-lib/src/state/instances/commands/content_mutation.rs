...
impl<'a> ContentMutationExecutor<'a> {
	...
	pub(super) async fn execute(
		...
		prepared
			.projection
			.apply(&self.state.content_store)
			.await?;
		...
		super::mark_shared_instance_stale(
			&self.instance.id,
			&self.state.pool,
		)
		.await?;
	...
	async fn prepare_install(
		...
		let requested_source = self
			...
			.await?;
		let legacy_path = if previous_path.is_some_and(|path| path != relative_path)
		{
			previous_path
	...
	async fn prepare_toggle(
		...
		let projection = match self.state.content_store.file_content(&file).await? {
			FileContent::Stored(blob) => {
					...
					.await?
				{
					ContentProjectionStatus::Healthy => {
						self.state
							.content_store
							.prepare_move(&self.instance, &file, &binding, enabled)
							.await?
					}
	...
	async fn prepare_adopt(
		...
		let canonical_record = content_rows::get_instance_file_by_relative_path(
			&self.instance.id,
			canonical,
			&self.state.pool,
		)
		...
		let result = match &prepared.change {
			PreparedChange::Install {
				...
				upsert_entry_for_file(
					content_scope.as_ref().expect("install requires content scope"),
					&file,
					*project_type,
