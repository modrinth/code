...
use crate::state::{
	CacheBehaviour, CachedEntry, CachedFile, ContentItem, ContentItemOwner,
	ContentItemProject, ContentItemVersion, Dependency, LinkedModpackInfo,
	Organization, OwnerType, ContentFile, Project, ProjectType,
...
pub(crate) async fn get_installed_project_ids_for_instance(
	...
	Ok(projects
		.into_iter()
		.filter_map(|(_, file)| file.metadata.map(|metadata| metadata.project_id))
		.collect::<HashSet<_>>()
		.into_iter()
		.collect())
}
...
pub(crate) async fn list_linked_modpack_content(
	...
	.await?;
	let Some((_, version_id)) = linked_modpack_ids_for_instance(
		&resolved.instance.id,
		&state.pool,
	)
	.await?
...
pub(crate) async fn get_linked_modpack_info(
	...
	.await?;
	let Some((project_id, version_id)) = linked_modpack_ids_for_instance(
		&resolved.instance.id,
		&state.pool,
	)
	.await?
...
pub(crate) async fn dependencies_to_content_items(
	...
	let mut items = dependencies
		...
		.filter_map(|dependency| {
			...
			let project = meta.projects.iter().find(|project| &project.id == project_id)?;
			...
			let owner = resolve_owner(project, &meta.teams, &meta.organizations);
			let project_type = project_type_from_api_name(&project.project_type);
...
async fn resolve_content_scope_with_instance(
	...
	let content_set = match content_set_id {
		Some(content_set_id) => {
			let content_set = sqlite::content_rows::get_content_set(
				content_set_id,
				pool,
			)
			.await?
		...
		None => sqlite::content_rows::get_applied_content_set(
			&instance.id,
			pool,
		)
		.await?
...
async fn content_files_to_content_items(
	...
	let project_ids = files
		.iter()
		.filter_map(|(_, file)| {
			file.metadata.as_ref().map(|metadata| metadata.project_id.clone())
		})
		.collect::<HashSet<_>>();
	let version_ids = files
		.iter()
		.filter_map(|(_, file)| {
			file.metadata.as_ref().map(|metadata| metadata.version_id.clone())
		})
		.collect::<HashSet<_>>();
	let meta = resolve_metadata(
		&project_ids,
...
async fn linked_modpack_ids_for_instance(
	...
	let link = sqlite::instance_rows::get_instance_link(instance_id, pool).await?;
...
fn check_modpack_update(
	...
	for version_types in effective_channel.version_type_fallbacks() {
		if !versions
			.iter()
			.any(|version| version_types.contains(&version.version_type.as_str()))
		{
			continue;
		}
...
impl ModpackIdentifiers {
	fn is_modpack_file(
		...
		self.hashes.contains(hash)
			|| entry_project_id.is_some_and(|project_id| {
				self.project_ids.contains(project_id)
			})
			|| file.is_some_and(|file| {
				self.project_ids.contains(&file.project_id)
			})
	}
}
