...
pub(crate) async fn add_project_from_version(
	...
	let content_set = content_rows::get_content_set(&scope.content_set_id, &state.pool)
		.await?
		.ok_or_else(|| {
			crate::ErrorKind::InputError(format!(
				"Unknown content set {}",
	...
	let version =
		CachedEntry::get_version(version_id, None, &state.pool, &state.api_semaphore)
			.await?
			.ok_or_else(|| {
				crate::ErrorKind::InputError(format!(
	...
	.await?;
	let project_type =
		ProjectType::get_from_loaders(version.loaders.clone()).ok_or_else(|| {
			crate::ErrorKind::InputError(format!(
...
pub(crate) async fn add_project_bytes(
	...
	let full_path = instance_full_path(state, &scope.instance).join(&relative_path);
	let sha1 = match hash {
...
pub(crate) async fn toggle_disable_project(
	...
	io::rename_or_move(&base.join(&current_path), &base.join(&new_path)).await?;
...
pub(crate) async fn list_project_files(
	...
	let entries =
		content_rows::get_content_entries(&scope.content_set_id, &state.pool)
			.await?;
	let files = content_rows::get_instance_files(&scope.instance.id, &state.pool)
		.await?
		.into_iter()
...
pub(crate) fn instance_full_path(state: &State, instance: &Instance) -> PathBuf {
...
fn infer_project_type(bytes: &Bytes) -> crate::Result<ProjectType> {
	...
	} else if archive.file_names().any(|name| name.starts_with("shaders/")) {
