...
use crate::state::{CacheBehaviour, CachedEntry, ProjectType, ReleaseChannel, State};
...
pub(crate) async fn check_content_updates(
	...
	let content_set = content_rows::get_applied_content_set(&instance.id, &state.pool)
		.await?
		.ok_or_else(|| {
	...
	let entries = content_rows::get_content_entries(&content_set.id, &state.pool)
		.await?;
	let files = content_rows::get_instance_files(&instance.id, &state.pool)
		.await?;
	...
	let update_keys = candidates
		...
		.collect::<Vec<_>>();
	let update_key_refs = update_keys.iter().map(|key| key.as_str()).collect::<Vec<_>>();
