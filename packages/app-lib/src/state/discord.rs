...
impl DiscordGuard {
	...
	pub fn init() -> crate::Result<DiscordGuard> {
		let dipc = DiscordIpcClient::new("1123683254248148992").map_err(|e| {
			crate::ErrorKind::OtherError(format!("Could not create Discord client {e}",))
		})?;
	...
	pub async fn force_set_activity(
		...
		let could_not_set_err = |e: Box<dyn serde::ser::StdError>| {
			crate::ErrorKind::OtherError(format!("Could not update Discord activity {e}",))
		};

		if reconnect_if_fail {
			if let Err(_e) = res {
				client.reconnect().map_err(|e| {
					crate::ErrorKind::OtherError(format!("Could not reconnect to Discord IPC {e}",))
				})?;
				return Ok(client.set_activity(activity).map_err(could_not_set_err)?); // try again, but don't reconnect if it fails again
			}
		} else {
	...
	pub async fn clear_activity(&self, reconnect_if_fail: bool) -> crate::Result<()> {
		...
		let could_not_clear_err = |e: Box<dyn serde::ser::StdError>| {
			crate::ErrorKind::OtherError(format!("Could not clear Discord activity {e}",))
		};
		...
		if reconnect_if_fail {
			if res.is_err() {
				...
				return Ok(client.clear_activity().map_err(could_not_clear_err)?); // try again, but don't reconnect if it fails again
			}
		} else {
	...
	pub async fn clear_to_default(&self, reconnect_if_fail: bool) -> crate::Result<()> {
		let state = State::get().await?;
