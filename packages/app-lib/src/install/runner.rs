...
use super::model::{
	InstallCleanup, InstallErrorView, InstallJobSnapshot, InstallJobState,
	InstallJobDisplay, InstallJobStatus, InstallPhaseDetails, InstallPhaseId,
	InstallPostInstallEdit, InstallRequest, InstallRollbackState, InstallTarget,
};
...
pub async fn retry_job(job_id: Uuid) -> crate::Result<InstallJobSnapshot> {
	...
	if !matches!(
		...
		return Err(crate::ErrorKind::InputError(
			"Only failed or interrupted install jobs can be retried".to_string(),
		)
		.into());
	}
	...
	let record =
		store::update_status(job_id, InstallJobStatus::Queued, &job.state, &state)
			.await?;
...
pub async fn cancel_job(job_id: Uuid) -> crate::Result<InstallJobSnapshot> {
	...
	let record =
		store::update_status(job_id, InstallJobStatus::Canceled, &job.state, &state)
			.await?;
	emit_install_job(&record.snapshot()).await?;
...
async fn prepare_initial_instance(
	...
	match job_state.request.clone() {
		...
		InstallRequest::DuplicateInstance { source_instance_id } => {
			let metadata = crate::state::get_instance(&source_instance_id, &state.pool)
				.await?
				.ok_or_else(|| {
		...
		| InstallRequest::InstallPackToExistingInstance { instance_id, .. } => {
...
fn spawn_job(job_id: Uuid) {
	tokio::spawn(async move {
		if let Err(error) = run_job(job_id).await {
			tracing::error!("Install job {job_id} failed to update state: {error}");
		}
	});
}
...
async fn run_job(job_id: Uuid) -> crate::Result<()> {
	...
	let record =
		store::update_status(job_id, InstallJobStatus::Running, &job_state, &state)
			.await?;
	emit_install_job(&record.snapshot()).await?;
	...
	match job_state.request.clone() {
		...
		InstallRequest::InstallPackToExistingInstance {
			...
			let disabled_project_ids =
				remove_existing_pack_content(job_id, job_state, state, &instance_id)
					.await?;
			install_pack(
				job_id,
				job_state,
			...
			.await?;
			restore_disabled_projects(&instance_id, disabled_project_ids, state)
				.await?;
			apply_post_install_edit(&instance_id, post_install_edit).await?;
			Ok(Some(instance_id))
		}
...
async fn remove_existing_pack_content(
	...
	let (project_id, version_id) = match &metadata.link {
		...
		InstanceLink::ImportedModpack { .. } => {
			remove_existing_imported_pack_content(instance_id, &metadata, state)
				.await?;
	...
	let disabled_project_ids =
		crate::state::instances::commands::list_project_files(instance_id, state)
			.await?
			.into_iter()
...
async fn remove_existing_imported_pack_content(
	...
	for entry in entries {
		if !matches!(
			entry.source_kind,
			ContentSourceKind::ImportedModpack | ContentSourceKind::ModrinthModpack
		) {
			continue;
		}
...
fn set_instance_id(job_state: &mut InstallJobState, instance_id: String) {
	job_state.target = match &job_state.target {
		InstallTarget::ExistingInstance { .. } => InstallTarget::ExistingInstance {
...
fn current_instance_id(job_state: &InstallJobState) -> Option<String> {
	match &job_state.target {
		...
		InstallTarget::ExistingInstance { instance_id } => Some(instance_id.clone()),
	}
}
