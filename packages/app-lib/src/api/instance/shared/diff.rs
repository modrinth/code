...
pub(super) async fn shared_instance_update_diffs(
	...
	let (version_ids, external_files) = current_shared_content(metadata, modpack_unlinked, state).await?;
	let before = SharedContentSnapshot { version_ids, external_files, configuration: before_configuration };
	let (version_ids, external_files) = remote_shared_content(version)?;
	let after = SharedContentSnapshot { version_ids, external_files, configuration: after_configuration };
...
pub(super) async fn shared_instance_publish_diffs(
	...
	let disabled_versions = async {
		if snapshot.disabled_version_ids.is_empty() {
			...
			shared_versions_by_project(&snapshot.disabled_version_ids, state).await
		}
	};
	...
	let after = SharedContentSnapshot {
		version_ids: snapshot.version_ids.iter()
			.filter(|id| after_configuration.modpack_version_id.as_deref() != Some(id.as_str()))
			.cloned().collect(),
		external_files: snapshot.external_files.iter()
			.map(|file| shared_external_file_key(&file.file_type, &file.file_name))
			.collect::<crate::Result<_>>()?,
		configuration: after_configuration,
	};
	let mut removed_disabled_project_ids = snapshot.disabled_project_ids.clone();
...
fn local_configuration(metadata: &crate::state::InstanceMetadata) -> ContentSetConfiguration {
	ContentSetConfiguration {
		modpack_version_id: shared_modpack_id(&metadata.link).filter(|id| !id.is_empty()),
...
fn remote_configuration(version: &InstanceVersionResponse) -> ContentSetConfiguration {
	ContentSetConfiguration {
		modpack_version_id: version.modpack_id.clone().filter(|id| !id.is_empty()),
...
async fn shared_configuration_diffs(
	...
	for change in changes {
		match change {
			ConfigurationDiff::Modpack(Change::Added { after }) => diffs.push(configuration_diff(
				SharedInstanceUpdateDiffType::ModpackLinked,
				None,
				shared_modpack_version_label(Some(&after), state).await,
			)),
			ConfigurationDiff::Modpack(Change::Removed { before }) => diffs.push(configuration_diff(
			...
			ConfigurationDiff::Modpack(Change::Updated { before, after }) => {
				let current = shared_modpack_version_details(&before, state).await;
				let new = shared_modpack_version_details(&after, state).await;
				let project_name = new.as_ref().and_then(|details| details.project_name.clone())
					.or_else(|| current.as_ref().and_then(|details| details.project_name.clone()));
				diffs.push(SharedInstanceUpdateDiff {
					type_: SharedInstanceUpdateDiffType::ModpackUpdated,
					project_id: None,
					project_name,
					file_name: None,
					current_version_name: current.map(|details| details.version_name),
					new_version_name: new.map(|details| details.version_name),
					config_file_count: None,
					disabled: false,
				});
			}
			ConfigurationDiff::GameVersion(change) => diffs.push(configuration_diff(
				SharedInstanceUpdateDiffType::GameVersionUpdated,
				change.before().cloned(),
				change.after().cloned(),
			)),
			ConfigurationDiff::Loader(change) => diffs.push(configuration_diff(
...
async fn shared_content_diffs(
	...
	let to_snapshot = |source: &SharedContentSnapshot, versions: &HashMap<String, crate::state::Version>| ContentSetSnapshot {
		projects: versions.iter().map(|(project_id, version)| (project_id.clone(), version.id.clone())).collect(),
	...
	let diff = diff_content_sets(
		...
		&ContentSetDiffOptions { common_external_files },
	).with_additional(diff_configuration(&before.configuration, &after.configuration));
	if !diff.has_changes() {
		return Ok(Vec::new());
	}
	let project_ids = diff.content.iter().filter_map(|entry| match entry {
		ContentSetDiffEntry::Project { project_id, .. } => Some(project_id.clone()),
		...
		match entry {
			ContentSetDiffEntry::Project { project_id, change } => {
				...
				content_diffs.push(SharedInstanceUpdateDiff {
					...
					project_name: Some(project_names.get(&project_id).cloned().unwrap_or_else(|| project_id.clone())),
					current_version_name: change.before().map(|id| before_versions.get(&project_id)
						.map(|version| version.version_number.clone()).unwrap_or_else(|| id.clone())),
					new_version_name: change.after().map(|id| after_versions.get(&project_id)
						.map(|version| version.version_number.clone()).unwrap_or_else(|| id.clone())),
			...
			ContentSetDiffEntry::ExternalFile { file, kind } => {
				let disabled = kind == ContentSetDiffKind::Removed && removed_disabled_external_files.contains(&file);
				content_diffs.push(SharedInstanceUpdateDiff {
	...
	content_diffs.sort_by(|a, b| a.project_name.as_deref().or(a.file_name.as_deref())
...
fn shared_update_diff_type(kind: ContentSetDiffKind) -> SharedInstanceUpdateDiffType {
	match kind {
...
pub(super) fn shared_external_file_key(file_type: &str, path: &str) -> crate::Result<ExternalFileKey> {
	Ok(ExternalFileKey {
		content_type: file_type.parse().map_err(|error: modrinth_content_management::Error| {
...
fn remote_shared_content(version: &InstanceVersionResponse) -> crate::Result<(Vec<String>, BTreeSet<ExternalFileKey>)> {
	...
	let external_files = version.external_files.iter()
