		...
		.or_else(|| {
			definition.and_then(|definition| {
				if definition.versioned_keys.is_empty() {
					definition.keys.first().map(|key| (*key).to_string())
				} else {
					physical_variant_for_version(definition, target_version)
...
pub(super) async fn apply_shared_settings_to_instance(
	...
	let applied =
		apply_shared_settings_to_document(metadata, &mut document, state).await?;
...
pub(super) async fn capture_instance_options(
	...
	let canonical_changed = if crate::state::shared_game_options_exist(
		&state.pool,
	)
	.await?
	{
		read_instance_changes_into_shared_settings(metadata, &document, state)
			.await?
	} else {
	...
	if canonical_changed {
		let _ =
			sync_other_participating_instances(state, Some(&metadata.instance.id))
				.await;
	}
