    ...
    let values: Translations = if legacy {
        ...
        serde_json::from_slice::<BTreeMap<String, serde_json::Value>>(bytes)?
            ...
            .filter_map(|(key, value)| {
                value.as_str().map(|s| (key, s.to_owned()))
            })
            .collect()
    };
    Ok(values
...
pub(super) fn inspect(
    ...
    if hasher.digest().to_string() != expected_hash {
        return Err(input_error(
            "Locale source changed before it could be indexed",
        ));
    }
...
fn inspect_archive<R: Read + Seek>(
    ...
    let mut archive =
        ZipArchive::new(reader).map_err(|e| input_error(e.to_string()))?;
    ...
    *entries -= archive.len();
    ...
    for name in &names {
        ...
        if filename == "deprecated.json" {
            if namespace == "minecraft" {
                let bytes =
                    read_entry(&mut archive, name, MAX_LANGUAGE_BYTES, budget)?;
        ...
        let Some(locale) =
            filename.strip_suffix(if legacy { ".lang" } else { ".json" })
        else {
            continue;
        };
        ...
        match parse_language(&bytes, legacy) {
            Ok(values) => bundle
                ...
                .extend(values),
            Err(error) => {
                tracing::debug!(%name, %error, "Skipping malformed mod language file")
            }
        }
    }
    ...
    for manifest in [
        ...
        let bytes =
            read_entry(&mut archive, manifest, MAX_LANGUAGE_BYTES, budget)?;
        ...
        let jars = value
            .get("jars")
            .or_else(|| value.pointer("/quilt_loader/jars"));
        for jar in jars.and_then(|v| v.as_array()).into_iter().flatten() {
            if let Some(path) = jar
                .as_str()
                .or_else(|| jar.get("file").and_then(|v| v.as_str()))
    ...
    for name in nested {
        if !names.contains(&name) || !name.ends_with(".jar") {
            continue;
        }
