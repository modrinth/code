...
pub(super) async fn discover_custom_settings(
	...
	let locale_snapshot = if document.effective_entries().keys().any(|key| {
		let id = setting_by_file_key(key).map(|s| s.id.to_owned()).unwrap_or_else(|| custom_setting_id(key));
    ...
    tx.commit().await?;
	if discovered { super::locales::queue_game_locale_index(); }
    Ok(discovered)
}
