...
#[tracing::instrument]
pub async fn finish_login(code: &str, flow: MinecraftLoginFlow) -> crate::Result<Credentials> {
	let state = State::get().await?;
...
pub async fn set_default_user(user: uuid::Uuid) -> crate::Result<()> {
	...
	let (_, mut user) = users.remove(&user).ok_or_else(|| {
		crate::ErrorKind::OtherError(format!("Tried to get nonexistent user with ID {user}"))
			.as_error()
	})?;
