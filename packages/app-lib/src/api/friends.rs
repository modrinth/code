...
pub async fn friends() -> crate::Result<Vec<UserFriend>> {
	...
	let friends = FriendsSocket::friends(&state.pool, &state.api_semaphore).await?;
...
pub async fn add_friend(user_id: &str) -> crate::Result<()> {
	let state = crate::State::get().await?;
	FriendsSocket::add_friend(user_id, &state.pool, &state.api_semaphore).await?;
...
pub async fn remove_friend(user_id: &str) -> crate::Result<()> {
	let state = crate::State::get().await?;
	FriendsSocket::remove_friend(user_id, &state.pool, &state.api_semaphore).await?;
