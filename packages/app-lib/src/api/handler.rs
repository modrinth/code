...
pub async fn handle_url(sublink: &str) -> crate::Result<CommandPayload> {
	Ok(match sublink.split_once('/') {
		...
		Some(("version", id)) => CommandPayload::InstallVersion { id: id.to_string() },
		// /modpack/{id}   -    Installs a modpack of modpack id
		Some(("modpack", id)) => CommandPayload::InstallModpack { id: id.to_string() },
		_ => {
...
pub async fn parse_command(command_string: &str) -> crate::Result<CommandPayload> {
