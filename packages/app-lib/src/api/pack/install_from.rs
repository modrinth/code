...
pub async fn generate_pack_from_version_id(
	...
	let version = CachedEntry::get_version(&version_id, None, &state.pool, &state.api_semaphore)
		.await?
		.ok_or_else(|| crate::ErrorKind::InputError("Invalid version ID specified!".to_string()))?;
	...
	.ok_or_else(|| crate::ErrorKind::InputError("Specified version has no files".to_string()))?;
	...
	let project =
		CachedEntry::get_project(&version.project_id, None, &state.pool, &state.api_semaphore)
			.await?
			.ok_or_else(|| {
				crate::ErrorKind::InputError("Invalid project ID specified!".to_string())
			})?;
