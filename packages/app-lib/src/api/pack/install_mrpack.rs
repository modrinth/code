...
use crate::state::{CacheBehaviour, CachedEntry, ProfileInstallStage, SideType, cache_file_hash};
		...
		loading_try_for_each_concurrent(
			...
			|project| {
				...
				async move {
					...
					let path = std::path::Path::new(&project_path).components().next();
		...
		for index in 0..zip_reader.file().entries().len() {
				...
				&& !filename.ends_with('/')
			{
				...
				if new_path.file_name().is_some() {
					...
					write(
						&profile::get_full_path(&profile_path).await?.join(new_path),
						&bytes,
						&state.io_semaphore,
					)
					.await?;
...
pub async fn remove_all_related_files(
	...
	if let Some(zip_index) = zip_index_option {
		...
		let profile = profile::get(&profile_path)
			.await?
			.ok_or_else(|| crate::ErrorKind::UnmanagedProfileError(profile_path.to_string()))?;
		...
		for index in 0..zip_reader.file().entries().len() {
				...
				&& !filename.ends_with('/')
			{
				...
				let existing_file = profile::get_full_path(&profile_path).await?.join(&new_path);
