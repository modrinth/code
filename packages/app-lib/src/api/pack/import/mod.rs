...
pub async fn get_importable_instances(
	...
	let instances_subfolder = match launcher_type {
		...
		ImportLauncherType::Unknown => {
			return Err(crate::ErrorKind::InputError("Launcher type Unknown".to_string()).into());
		}
	};
...
#[tracing::instrument]
pub async fn recache_icon(icon_path: PathBuf) -> crate::Result<Option<PathBuf>> {
...
pub async fn copy_dotminecraft(
	...
	let profile_path = crate::api::profile::get_full_path(profile_path_id).await?;
	...
	for src_child in subfiles {
		let dst_child = src_child.strip_prefix(&dotminecraft).map_err(|_| {
			crate::ErrorKind::InputError(format!("Invalid file: {}", &src_child.display()))
		})?;
...
#[tracing::instrument]
pub async fn get_all_subfiles(src: &Path, include_empty_dirs: bool) -> crate::Result<Vec<PathBuf>> {
	if !src.is_dir() {
