...
use crate::event::emit::{emit_loading, emit_instance, init_loading};
use crate::event::{LoadingBarType, InstancePayloadType};
use crate::pack::install_from::{
	EnvType, PackDependency, PackFile, PackFileHash, PackFormat,
};
use crate::server_address::ServerAddress;
use crate::state::{
	CacheBehaviour, CachedEntry, ContentItem, ContentSet, CreateInstance,
	Credentials, Dependency, EditInstance, ContentFile, InstanceLink,
	InstanceMetadata, JavaVersion, LinkedModpackInfo, ModLoader,
	ProcessMetadata, InstanceInstallStage, ProjectType, Settings,
	SideType, State,
};
...
pub async fn remove(instance_id: &str) -> crate::Result<()> {
	...
	if let Some(metadata) = metadata {
		emit_instance(&metadata.instance.id, InstancePayloadType::Removed).await?;
	}
...
pub async fn get_full_path(instance_id: &str) -> crate::Result<PathBuf> {
	...
	Ok(io::canonicalize(
		state.directories.instances_dir().join(metadata.instance.path),
	)?)
}
...
pub async fn get_optimal_jre_key(
	...
	let (minecraft, version_index) = crate::launcher::resolve_minecraft_manifest(
...
pub async fn install(instance_id: &str, force: bool) -> crate::Result<()> {
	...
	let result = crate::launcher::install_minecraft(&context, None, force).await;
	if result.is_err() {
		let current_stage =
			crate::state::instances::commands::get_instance_launch_context(
				instance_id,
...
pub async fn update_all_projects(
	...
	.await?;
	let map =
		crate::state::instances::commands::update_all_projects(instance_id, &state)
			.await?;
...
pub async fn add_project_from_version(
	...
	let project_path = crate::state::instances::commands::add_project_from_version(
		instance_id,
		version_id,
		reason,
...
#[tracing::instrument]
pub async fn remove_project(instance_id: &str, project: &str) -> crate::Result<()> {
	let state = State::get().await?;
	let metadata = get(instance_id).await?.ok_or_else(|| {
		crate::ErrorKind::InputError("Unknown instance".to_string())
	})?;
...
pub async fn create_mrpack_json(
	...
	match (
		...
		(ModLoader::Forge, Some(v)) => dependencies.insert(PackDependency::Forge, v),
