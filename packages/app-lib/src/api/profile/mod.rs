...
#[tracing::instrument]
pub async fn get_mod_full_path(profile_path: &str, project_path: &str) -> crate::Result<PathBuf> {
...
pub async fn edit<Fut>(path: &str, action: impl Fn(&mut Profile) -> Fut) -> crate::Result<()>
where
...
pub async fn edit_icon(path: &str, icon_path: Option<&Path>) -> crate::Result<()> {
	let state = State::get().await?;
...
pub async fn get_optimal_jre_key(path: &str) -> crate::Result<Option<JavaVersion>> {
	let state = State::get().await?;
...
pub async fn install(path: &str, force: bool) -> crate::Result<()> {
	if let Some(profile) = get(path).await? {
		let result = crate::launcher::install_minecraft(&profile, None, force).await;
...
#[tracing::instrument]
pub async fn update_all_projects(profile_path: &str) -> crate::Result<HashMap<String, String>> {
...
#[tracing::instrument]
pub async fn toggle_disable_project(profile_path: &str, project: &str) -> crate::Result<String> {
...
/// Uses and returns the relative path to the project
#[tracing::instrument]
pub async fn remove_project(profile_path: &str, project: &str) -> crate::Result<()> {
...
#[tracing::instrument]
pub async fn get_pack_export_candidates(profile_path: &str) -> crate::Result<Vec<String>> {
...
fn pack_get_relative_path(profile_path: &PathBuf, path: &PathBuf) -> crate::Result<String> {
	Ok(path
		...
		.map_err(|_| {
			crate::ErrorKind::FSError(format!("Path {path:?} does not correspond to a profile"))
		})?
		.components()
