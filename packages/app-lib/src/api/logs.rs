	...
	pub fn censor(mut s: String, credentials_list: &[Credentials]) -> Self {
		...
		for credentials in credentials_list {
			...
			s = s
				...
				.replace(&credentials.offline_profile.name, "{MINECRAFT_USERNAME}")
				.replace(
	...
	async fn build(
		...
		Ok(Self {
			...
			output: if clear_contents.unwrap_or(false) {
				...
				Some(get_output_by_filename(profile_subpath, log_type, &filename).await?)
			},
			filename,
		})
	}
...
pub async fn get_logs_from_type(
	...
	let logs_folder = match log_type {
		...
		LogType::CrashReport => state.directories.crash_reports_dir(profile_path),
	};
	...
	if logs_folder.exists() {
			...
			std::fs::read_dir(&logs_folder).map_err(|e| IOError::with_path(e, &logs_folder))?
		{
			...
			if let Some(file_name) = path.file_name() {
				...
				logs.push(
					Logs::build(log_type, age, profile_path, file_name, clear_contents).await,
				);
			}
		}
	}
...
pub async fn get_logs(
	...
	get_logs_from_type(profile_path_id, LogType::InfoLog, clear_contents, &mut logs).await?;
	get_logs_from_type(
		profile_path_id,
		LogType::CrashReport,
		clear_contents,
...
pub async fn get_output_by_filename(
	...
	let logs_folder = match log_type {
		...
		LogType::CrashReport => state.directories.crash_reports_dir(profile_subpath),
	};
	...
	Err(
		crate::ErrorKind::OtherError(format!("File extension not supported: {}", path.display()))
			.into(),
	)
}
...
pub async fn delete_logs_by_filename(
	...
	let logs_folder = match log_type {
		...
		LogType::CrashReport => state.directories.crash_reports_dir(profile_path_id),
	};
