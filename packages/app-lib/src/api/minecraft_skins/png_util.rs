...
use crate::{ErrorKind, minecraft_skins::UrlOrBlob, util::fetch::REQWEST_CLIENT};
...
pub async fn normalize_skin_texture(texture: &UrlOrBlob) -> crate::Result<Bytes> {
	...
	tokio::task::spawn_blocking(|| {
		...
		if let Some(source_chromacities) = png_reader.info().source_chromaticities.as_ref().copied()
		{
		...
		macro_rules! do_copy {
			($pixel_type:ty) => {
				copy_rect_mirror_horizontally::<$pixel_type>(
					...
					::bytemuck::try_cast_slice_mut(texture_buf)
...
async fn normalize_skin_texture_works() {
	...
	let expected_normalized_png_data = &include_bytes!("assets/test/MissingNo_normalized.png")[..];

	let normalized_png_data = normalize_skin_texture(&UrlOrBlob::Blob(legacy_png_data.into()))
		.await
	...
	let (normalized_pixels, normalized_info) = decode_to_pixels(&normalized_png_data);
	let (expected_pixels, expected_info) = decode_to_pixels(expected_normalized_png_data);
