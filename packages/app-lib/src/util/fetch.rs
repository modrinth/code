			...
			Ok(x) => {
				if x.status().is_server_error() {
					if attempt <= FETCH_ATTEMPTS {
						...
						return Err(crate::Error::from(crate::ErrorKind::OtherError(
							"Server error when fetching content".to_string(),
						)));
					}
				}
					...
					if let Some(sha1) = sha1 {
						...
						if &*hash != sha1 {
							if attempt <= FETCH_ATTEMPTS {
								...
								return Err(
									crate::ErrorKind::HashError(sha1.to_string(), hash).into()
								);
							}
						}
					}
...
pub async fn fetch_mirrors(
	...
	if mirrors.is_empty() {
		return Err(crate::ErrorKind::InputError("No mirrors provided!".to_string()).into());
	}
	...
	T: DeserializeOwned,
{
	...
	if let Some(creds) = crate::state::ModrinthCredentials::get_active(exec).await? {
		req = req.header("Authorization", &creds.session);
	}
...
pub async fn write(path: &Path, bytes: &[u8], semaphore: &IoSemaphore) -> crate::Result<()> {
	let _permit = semaphore.0.acquire().await?;
