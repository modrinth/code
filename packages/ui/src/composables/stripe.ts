...
export const useStripe = (
        ...
        try {
            ...
            if (!selectedPaymentMethod.value) {
                ...
                const {
                    ...
                    amount: product.value?.prices.find((x) => x.currency_code === currency)?.prices
    ...
    async function createConfirmationToken() {
        ...
        const { error, confirmationToken: confirmation } =
    ...
    async function createNewPaymentMethod() {
        ...
        if (submitError) {
            return handlePaymentError(
                submitError.message ?? 'Unknown error creating payment method',
            )
        }
