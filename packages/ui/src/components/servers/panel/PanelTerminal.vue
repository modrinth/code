  ...
  <div class="contents">
    <div class="flex items-center gap-4">
      <div class="relative w-full">
        ...
        <span
          v-if="serverConsole.filteredOutput.value.length && searchInput && !loading"
          class="pointer-events-none absolute right-12 top-1/2 -translate-y-1/2 select-none whitespace-pre text-sm"
        >
          {{ serverConsole.filteredOutput.value.length }}
          {{ serverConsole.filteredOutput.value.length === 1 ? 'result' : 'results' }}
        </span>
      </div>
    </div>
    <div
    ...
    >
      <div
        v-if="useAdvancedRendering && !loading"
...
const serverConsole = injectModrinthServersConsole()
const consoleOutput = computed(() => (props.loading ? [] : serverConsole.output.value))
...
const updateSearch = useDebounceFn((value: string) => {
  serverConsole.setSearchQuery(value)
  ...
  nextTick(() => {
    ...
    if (serverConsole.filteredOutput.value.length > 0) {
...
const clearSearch = () => {
  searchInput.value = ''
  serverConsole.setSearchQuery('')
}

const activeOutput = computed(() => {
  return searchInput.value ? serverConsole.filteredOutput.value : consoleOutput.value
})
...
const jumpToLine = (line: string, event?: MouseEvent) => {
  ...
  const index = serverConsole.findLineIndex(line)
  if (index === -1) return

  const filteredLineIndex = serverConsole.filteredOutput.value.indexOf(line)
  ...
  nextTick(() => {
    ...
    const targetScrollTop = Math.max(0, targetPosition - containerHeight * targetLineRelativeOffset)
