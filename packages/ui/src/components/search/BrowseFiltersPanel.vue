<template>
  <div>
    <Accordion v-for="filter in filters" :key="filter.id" v-model="filters" v-bind="$attrs" :button-class="buttonClass" :content-class="contentClass" open-by-default>
      <template #title>
  ...
  const filters: FilterType<any>[] = [
    {
      ...
      options: props.platforms.filter((x) => x.default && x.supported_project_types.includes('modpack')).map((x) => ({
        id: x.name,
    ...
    },
    {
      ...
      options: props.gameVersions?.filter((x) => x.major && x.version_type === 'release').map((x) => ({
        id: x.version,
