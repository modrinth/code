<template>
    ...
    >
        ...
        <template v-if="locked" #default>
            <div
                class="flex flex-col gap-2 p-3 border-dashed border-2 rounded-2xl border-divider mx-2"
            >
...
function isVisible(filter: FilterOption) {
    ...
    if (props.filterType.display === 'expandable') {
        return (
            matchesQuery && (showMore.value || props.filterType.default_values.includes(filter.id))
        )
    }
...
function isProvided(filter: FilterOption, negative: boolean) {
    return props.providedFilters.some(
        (x) =>
            x.type === props.filterType.id && x.option === filter.id && !x.negative === !negative,
    )
}
