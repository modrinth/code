...
<template>
  <div class="grid grid-cols-[min-content,4fr,2fr,3fr,min-content] gap-2 items-center rounded-xl bg-bg-raised p-2">
    <Checkbox v-model="selected" :description="``" class="select-checkbox" @update:model-value="emit('update:selected', $event)" />
    <AutoLink :to="item.project?.link ?? ''" v-bind="item.project?.linkProps ?? {}">
      <div class="flex items-center gap-2 text-contrast font-medium" :class="{ 'opacity-50': item.disabled}">
