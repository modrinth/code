...
<template>
  <div class="flex flex-col grid-cols-[min-content,auto,auto,auto,auto] gap-2">
    <div
      class="grid grid-cols-[min-content,4fr,2fr,3fr,auto] gap-2 items-center px-2 pb-2 text-contrast font-bold"
    >
      ...
      <div class="flex w-[36px]"></div>
    </div>
    <ContentListItem
      v-for="item in items.slice().sort((a, b) => a.filename.localeCompare(b.filename))"
      :item="item"
    >
