    ...
    <NewModal ref="purchaseModal">
        ...
        <div class="flex items-center gap-1 pb-4">
            ...
            <span
                :class="{
                    'text-secondary':
                        purchaseModalStep !== (productType === 'pyro' && !projectId ? 1 : 0),
                    'font-bold':
                        purchaseModalStep === (productType === 'pyro' && !projectId ? 1 : 0),
                }"
            >
            ...
            <span
                :class="{
                    'text-secondary':
                        purchaseModalStep !== (productType === 'pyro' && !projectId ? 2 : 1),
                    'font-bold':
                        purchaseModalStep === (productType === 'pyro' && !projectId ? 2 : 1),
                }"
            >
            ...
            <span
                :class="{
                    'text-secondary':
                        purchaseModalStep !== (productType === 'pyro' && !projectId ? 3 : 2),
                    'font-bold':
                        purchaseModalStep === (productType === 'pyro' && !projectId ? 3 : 2),
                }"
            >
                ...
                <div class="flex flex-col gap-4">
                    ...
                    >
                        ...
                        <div class="flex flex-col w-full gap-2">
                            ...
                            <input
                                :value="mutatedProduct.metadata.cpu"
                                disabled
                                class="input w-full"
                            />
                        </div>
                    ...
                    >
                        We are currently
                        <a :href="outOfStockUrl" class="underline" target="_blank"
                            >out of capacity</a
                        >
                        for your selected RAM amount. Please try again later, or try a different
                        amount.
                    </Admonition>
                    <Admonition
                        v-else
                        type="info"
                        header="We can't seem to find your selected plan"
                    >
                        We are currently unable to find a server for your selected RAM amount.
                        Please try again later, or try a different amount.
                    </Admonition>

                    <div class="flex gap-2">
                        <InfoIcon class="hidden sm:block shrink-0 mt-1" />
                        <span class="text-sm text-secondary">
                            Storage and shared CPU count are currently not configurable
                            independently, and are based on the amount of RAM you select.
                        </span>
                    </div>
                </div>
            </div>
        ...
        <div
            v-if="
                purchaseModalStep ===
                (mutatedProduct.metadata.type === 'pyro' && !projectId ? 1 : 0)
            "
            class="md:w-[600px]"
        >
            <div>
                ...
                <div v-if="existingPlan" class="flex flex-col gap-3 mb-4 text-secondary">
                    <p class="m-0">
                        The prices below reflect the new <strong>renewal cost</strong> of your
                        upgraded subscription.
                    </p>
                    <p class="m-0">
                        Today, you will be charged a prorated amount for the remainder of your
                        current billing cycle.
                    </p>
                </div>
                <div class="flex flex-col gap-4">
                    <div
                        v-for="([interval, rawPrice], index) in Object.entries(
                            price.prices.intervals,
                        )"
                        :key="index"
                    ...
                    <div class="flex items-baseline gap-2">
                        <span class="text-2xl font-extrabold text-primary">
                            {{
                                formatPrice(
                                    locale,
                                    price.prices.intervals[selectedPlan],
                                    price.currency_code,
                                )
                            }}
                        </span>
        ...
        <template
            v-if="
                purchaseModalStep ===
                (mutatedProduct.metadata.type === 'pyro' && !projectId ? 2 : 1)
            "
        >
            <div
        ...
        <div
            v-if="
                purchaseModalStep ===
                (mutatedProduct.metadata.type === 'pyro' && !projectId ? 3 : 2)
            "
            class="md:w-[650px]"
        >
            <div
                ...
                <div class="r-4 rounded-xl bg-bg p-4">
                    ...
                    <div class="mb-2 flex justify-between">
                        <span class="text-secondary">
                            {{
                                mutatedProduct.metadata.type === 'midas'
                                    ? 'Modrinth+'
                                    : 'Modrinth Servers'
                            }}
                            {{
                ...
                >
                    ...
                    <template #singleLabel="props">
                        <div class="flex items-center gap-2">
                            ...
                            <CurrencyIcon
                                v-else-if="props.option.type === 'cashapp'"
                                class="h-8 w-8"
                            />
                            <PayPalIcon
                                v-else-if="props.option.type === 'paypal'"
                                class="h-8 w-8"
                            />
                            ...
                            <span v-if="props.option.type === 'card'">
                                {{
                                    formatMessage(messages.paymentMethodCardDisplay, {
                                        card_brand:
                                            formatMessage(
                                                paymentMethodTypes[props.option.card.brand],
                            ...
                            <span
                                v-if="
                                    props.option.type === 'cashapp' && props.option.cashapp.cashtag
                                "
                            >
            ...
            <p class="m-0 mt-9 text-sm text-secondary">
                <strong
                    >By clicking "Subscribe", you are purchasing a recurring subscription.</strong
                >
                <br />
                You'll be charged
                {{
            ...
            <template v-if="purchaseModalStep === 0">
                ...
                >
                    ...
                    <template v-else>
                        <RightArrowIcon />
                        {{
                            mutatedProduct.metadata.type === 'pyro' && !projectId
                                ? 'Next'
                                : 'Select'
                        }}
                    </template>
...
defineExpose({
    show: () => {
        ...
        selectedPlan.value = props.existingSubscription
            ? props.existingSubscription.interval
            : 'yearly'
