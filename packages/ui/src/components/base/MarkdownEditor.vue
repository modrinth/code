  ...
  const eventHandlers = EditorView.domEventHandlers({
    paste: (ev, view) => {
      ...
      if (clipboardData.files && clipboardData.files.length > 0 && props.onImageUpload) {
        ...
        uploadImageFromFile(Array.from(clipboardData.files)[0])
          .then(function (url) {
...
async function uploadImageFromFile(file: File): Promise<string> {
...
async function handleImageUpload(files: File[]) {
  if (props.onImageUpload && files.length > 0) {
    try {
      linkUrl.value = await uploadImageFromFile(files.at(-1)!)
      validateURL()
    } catch (error) {
      if (error instanceof Error) {
