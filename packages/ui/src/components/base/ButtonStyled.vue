...
const paddingX = computed(() => {
  if (props.size === 'large') {
    return props.circular ? '0.75rem' : '1rem'
  }
  return props.circular ? '0.5rem' : '0.75rem'
})
...
function setColorFill(colors: { bg: string, text: string }, fill: 'background' | 'text' | 'none'): { bg: string, text: string } {
  if (colorVar.value) {
    if (fill === 'background') {
...
const colorVariables = computed(() => {
  ...
  if (props.type === 'outlined' || props.type === 'transparent') {
    ...
    hoverColors = setColorFill(hoverColors, props.hoverColorFill === 'auto' ? 'text' : props.hoverColorFill)
  } else {
    colors = setColorFill(colors, props.colorFill === 'auto' ? 'background' : props.colorFill)
    hoverColors = setColorFill(hoverColors, props.hoverColorFill === 'auto' ? 'background' : props.hoverColorFill)
  }
...
<template>
  <div class="btn-wrapper" :class="{'outline': type === 'outlined'}" :style="`${colorVariables}--_height:${height};--_width:${width};--_radius: ${radius};--_padding-x:${paddingX};--_padding-y:${paddingY};--_gap:${gap};--_font-weight:${fontWeight};--_icon-size:${iconSize};`">
    <slot />
  </div>
</template>
