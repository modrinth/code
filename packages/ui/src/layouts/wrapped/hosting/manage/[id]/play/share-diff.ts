...
export function resolveServerShareDiff(diff: Archon.Content.v1.SharedInstancePublishDiff): ContentDiffItem[] {
	...
	return diff.diffs.map((entry): ContentDiffItem => {
		...
		const projectId = entry.type === 'project' ? entry.project_id : versionId ? diff.versions?.[versionId]?.project_id : undefined
		return {
