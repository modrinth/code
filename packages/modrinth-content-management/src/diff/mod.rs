...
pub fn diff_content_sets(
	...
	let project_ids = before.projects.keys().chain(after.projects.keys()).collect::<BTreeSet<_>>();
	for project_id in project_ids {
		if let Some(change) = Change::between(before.projects.get(project_id), after.projects.get(project_id)) {
	...
	for file in before.external_files.union(&after.external_files) {
		let kind = match (before.external_files.contains(file), after.external_files.contains(file)) {
			(false, true) => ContentSetDiffKind::Added,
			(true, false) => ContentSetDiffKind::Removed,
			(true, true) if options.common_external_files == CommonExternalFilePolicy::AssumeUpdated => ContentSetDiffKind::Updated,
			_ => continue,
		};
		content.push(ContentSetDiffEntry::ExternalFile { file: file.clone(), kind });
	}
	ContentSetDiff { content, additional: Vec::new() }
}
