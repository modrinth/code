use modrinth_content_management::{
	...
	ContentSetSnapshot, ContentType, Error, ExternalFileKey, LoaderReference, diff_configuration,
	diff_content_sets,
};
...
fn file(content_type: ContentType, path: &str) -> ExternalFileKey {
	ExternalFileKey { content_type, path: path.to_string() }
}
...
fn empty_and_identical_snapshots_have_no_changes() {
	for content in [
		...
		let diff = diff_content_sets(&content, &content, &ContentSetDiffOptions::default());
...
fn project_changes_preserve_identity_and_both_version_ids() {
	...
	let diff = diff_content_sets(&before, &after, &ContentSetDiffOptions::default());

	assert!(diff.has_changes());
	assert_eq!(diff.content, vec![
		ContentSetDiffEntry::Project {
			project_id: "added".to_string(),
			change: Change::Added { after: "v3".to_string() },
		},
		ContentSetDiffEntry::Project {
			project_id: "removed".to_string(),
			change: Change::Removed { before: "v2".to_string() },
		},
		ContentSetDiffEntry::Project {
...
fn reversing_snapshots_reverses_project_and_file_changes() {
	...
	let reverse = diff_content_sets(&after, &before, &ContentSetDiffOptions::default());

	assert_eq!(reverse.content, vec![
		ContentSetDiffEntry::Project {
			project_id: "added".to_string(),
			change: Change::Removed { before: "v2".to_string() },
		},
		ContentSetDiffEntry::Project {
			project_id: "removed".to_string(),
			change: Change::Added { after: "v1".to_string() },
		},
		ContentSetDiffEntry::Project {
			project_id: "updated".to_string(),
			change: Change::Updated {
...
fn common_file_policy_only_affects_files_present_on_both_sides() {
	...
	for (policy, expected) in [
		(CommonExternalFilePolicy::AssumeUnchanged, vec![added.clone(), removed.clone()]),
		(CommonExternalFilePolicy::AssumeUpdated, vec![added, common, removed]),
	] {
		let diff = diff_content_sets(&before, &after, &ContentSetDiffOptions {
...
fn assumed_common_file_update_counts_as_a_change() {
	...
	let diff = diff_content_sets(&content, &content, &ContentSetDiffOptions {
		common_external_files: CommonExternalFilePolicy::AssumeUpdated,
	});

	assert!(diff.has_changes());
	assert_eq!(diff.content, vec![ContentSetDiffEntry::ExternalFile {
...
fn conflicting_project_version_is_rejected_without_changing_the_snapshot() {
	...
	let error = content.insert_project("sodium".to_string(), "v2".to_string()).unwrap_err();

	assert!(matches!(error, Error::ConflictingProjectVersions { project_id, before, after }
...
fn caller_entries_can_trigger_changes_without_content_changes() {
	...
	let base = diff_content_sets(&content, &content, &ContentSetDiffOptions::default());
	let mut diff = base.with_additional(Vec::<AdditionalChange>::new());
	assert!(!diff.has_changes());

	diff.additional.push(AdditionalChange::ConfigFilesUpdated { file_count: 2 });

	assert!(diff.content.is_empty());
	assert!(diff.has_changes());
	assert_eq!(diff.additional, vec![AdditionalChange::ConfigFilesUpdated { file_count: 2 }]);
}
...
fn attaching_caller_entries_preserves_content_changes() {
	...
	let base = diff_content_sets(&before, &after, &ContentSetDiffOptions::default());
	let content = base.content.clone();
	let mut diff = base.with_additional([
		AdditionalChange::ConfigFilesUpdated { file_count: 1 },
	]);
...
fn modpack_link_unlink_and_update_have_directional_version_ids() {
	for (before_id, after_id, expected) in [
		(None, Some("pack-v1"), Change::Added { after: "pack-v1".to_string() }),
		(Some("pack-v1"), None, Change::Removed { before: "pack-v1".to_string() }),
		(Some("pack-v1"), Some("pack-v2"), Change::Updated {
			before: "pack-v1".to_string(),
			after: "pack-v2".to_string(),
		}),
	] {
		let before = configuration(before_id, "1.21.1", "fabric", Some("0.16.0"));
		let after = configuration(after_id, "1.21.1", "fabric", Some("0.16.0"));

		assert_eq!(diff_configuration(&before, &after), vec![ConfigurationDiff::Modpack(expected)]);
	}
}
...
fn configuration_normalization_does_not_create_false_changes() {
	...
	let linked = configuration(Some("pack-v1"), "1.21.1", "NeoForge", Some("21.1.1"));
	let canonical = configuration(Some("pack-v1"), "1.21.1", "neoforge", Some("21.1.1"));
...
fn changing_only_the_game_version_produces_one_configuration_entry() {
	...
	assert_eq!(diff_configuration(&before, &after), vec![
...
fn simultaneous_configuration_changes_are_composed_in_stable_order() {
	let before = configuration(Some("pack-v1"), "1.21.1", "fabric", Some("0.16.0"));
	let after = configuration(Some("pack-v2"), "1.21.2", "neoforge", Some("21.2.1"));
	let content = ContentSetSnapshot::default();
	let diff = diff_content_sets(&content, &content, &ContentSetDiffOptions::default())
...
fn mapping_changes_preserves_their_kind_and_before_after_values() {
	for (change, kind, before, after) in [
		(Change::Added { after: 2 }, ContentSetDiffKind::Added, None, Some("v2")),
		(Change::Removed { before: 1 }, ContentSetDiffKind::Removed, Some("v1"), None),
		(Change::Updated { before: 1, after: 2 }, ContentSetDiffKind::Updated, Some("v1"), Some("v2")),
	] {
