...
use quote::{
    IdentFragment, ToTokens, TokenStreamExt, format_ident, quote, quote_spanned,
};
use std::collections::HashMap;
use std::mem;
use syn::parse::{Parse, ParseStream};
use syn::{
    Attribute, Data, DeriveInput, Error, Fields, Ident, Index, LitStr, Member,
    Result, Token, Variant, parenthesized,
};
...
pub fn generate_impls(input: DeriveInput) -> Result<TokenStream> {
    ...
    let message_cases = variants.iter().map(|variant| {
        ...
        let message_key = format!(
            "{i18n_root_key}.{}",
            variant.translation_id.as_ref().unwrap().value()
        );
        let params = variant
            .translate_fields
            .as_ref()
...
fn find_i18n_root_key(
    ...
    Err(Error::new(
        enum_name.span(),
        "Missing #[i18n_root_key] attribute",
    ))
}
...
fn parse_variants(
    ...
    let mut result = variants.into_iter().map(VariantData::parse).collect();
...
impl VariantData {
    fn parse(variant: Variant) -> Self {
        ...
        Self {
            translation_id: translation_id.unwrap_or_else(|| {
                Err(Error::new(
                    name.span(),
...
impl Parse for TranslateFields {
    fn parse(input: ParseStream) -> Result<Self> {
        let result = input.parse_terminated(
            |input| {
    ...
    fn to_tokens(&self, tokens: &mut proc_macro2::TokenStream) {
        let member = match &self.member {
            ...
            Member::Unnamed(Index { index, span }) => {
                format_ident!("_{index}", span = *span).to_token_stream()
            }
        };
        let span = self.span;
        if self.translated {
...
fn validate_variants(variants: &mut Vec<VariantData>) -> Result<()> {
    fn take_err<T>(
        ...
        match value {
            Ok(_) => None,
            Err(e) => {
                Some(mem::replace(e, Error::new(variant_name.span(), "")))
            }
        }
    }
