...
async function compileArticles() {
	...
	for (const file of files) {
		...
		// Transform video tags into custom elements for Vue component hydration
		const defaultHtmlBlockRenderer =
			mdIt.renderer.rules.html_block ||
			function (tokens, idx, options, _env, self) {
				return self.renderToken(tokens, idx, options)
			}
		...
		mdIt.renderer.rules.html_block = (
			...
			// Check if this HTML block contains a video tag
			const videoTagMatch = content.match(/<video([^>]*)>([\s\S]*?)<\/video>/i)
			if (videoTagMatch) {
				const [, attributes, innerContent] = videoTagMatch

				// Extract src from attributes or source tag
				let src = ''
				const srcAttrMatch = attributes.match(/src=["']([^"']+)["']/i)
				if (srcAttrMatch) {
					src = srcAttrMatch[1]
				} else {
					// Check for source tag inside
				...
				if (src) {
					...
					const normalizedSrc = new URL(src, `${SITE_URL}/news/article/${slug}/`).href.replace(
						SITE_URL,
						'',
					)
					...
					const attrMatches = attributes.matchAll(/(\w+)=["']([^"']+)["']/gi)
					...
					// Return custom marker element
					return `<modrinth-video data-src="${normalizedSrc}" ${dataAttrs}></modrinth-video>`
				}
			}
