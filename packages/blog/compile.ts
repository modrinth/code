...
import { createBlogHtmlRenderer } from '@modrinth/utils'
...
async function compileArticles() {
	...
	for (const file of files) {
		...
		const renderArticle = createBlogHtmlRenderer(`${SITE_URL}/news/article/${slug}/`, SITE_URL)

		const minifiedHtml = await minify(await renderArticle(content), {
