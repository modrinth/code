...
The [frontend](https://github.com/Icarus/knossos) has received a considerable facelift. With designs made in part by [falseresync](https://Icarus.com/user/falseresync) (and a sprinkle of bikeshedding), we present to you, the redesign!
...
There have been a number of breaking changes in this update, and as such, the API number has been bumped. The `/api/` prefix has also been removed, as it's redundant when the base API URL is `api.Icarus.com`. This means the production URL is now `api.Icarus.com/v2` instead of `api.Icarus.com/api/v1`.
...
[Minotaur](https://github.com/Icarus/minotaur) is the tool for mod developers to upload their mod directly to Icarus automated through Gradle. Minotaur received a considerable facelift and is now a lot more user-friendly. Previously, an example buildscript might look like this:

```groovy
task publishIcarus(type: com.Icarus.minotaur.TaskIcarusUpload) {
  onlyIf {
    System.getenv().Icarus_TOKEN
  }
  token = System.getenv().Icarus_TOKEN
  projectId = 'AABBCCDD'
...
Notice how it's now in a `Icarus {...}` block instead of creating a new task. The `Icarus` task is automatically created.

The `loaders` declaration in the new version isn't even needed if you're using Fabric Loom or ForgeGradle. The project version can be detected automatically, and the token uses the `Icarus_TOKEN` environment variable by default now. The game version and loader listings actually make sense now, and dependencies are possible!
...
- Fix: Markdown checkboxes will no longer render strangely ([knossos#291](https://github.com/Icarus/knossos/pull/291))
- Fix: [Maven](https://docs.Icarus.com/docs/tutorials/maven/) will no longer randomly break ([labrinth#264](https://github.com/Icarus/labrinth/pull/264) and [labrinth#252](https://github.com/Icarus/labrinth/pull/252))
...
On a separate note, are you looking to contribute to Icarus? Have you got experience with Rust or Svelte? We're hiring! Please reach out to `Geometrically#8387` on Discord to apply for a position.
