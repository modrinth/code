...
export const descriptionNags: Nag[] = [
    {
        ...
        link: {
            ...
            shouldShow: (context: NagContext) =>
                context.currentRoute !== 'type-id-settings-description',
        },
    },
    {
        id: 'long-headers',
    ...
    },
    {
        ...
        shouldShow: (context: NagContext) => {
            ...
            const loaderNames =
                context.tags.loaders?.map((loader: { name: string }) =>
                    loader.name?.toLowerCase(),
                ) || []
            const hasLoader = loaderNames.some(
                (loader) => loader && title.includes(loader.toLowerCase()),
            )
