...
function getCategories(
    ...
    return (
        tags.categories?.filter(
            (category: { project_type: string }) =>
                category.project_type === project.actualProjectType,
        ) ?? []
    )
}
...
export const tagsNags: Nag[] = [
    {
        ...
        description: (context: NagContext) => {
            ...
            const tagCount =
                context.project.categories.length +
                (context.project.additional_categories?.length || 0)
