...
export default stage(
	...
	[
		linkSection('issues', ({ project }) => `**Issues:** ${mdEscape(project.link_urls.issues?.url ?? '')}`, 500)
			.shown(({ project }) => !!project.link_urls.issues?.url),

		linkSection('source', ({ project }) => `**Source:** ${mdEscape(project.link_urls.source?.url ?? '')}`, 510)
			.shown(({ project }) => !!project.link_urls.source?.url),

		linkSection('wiki', ({ project }) => `**Wiki:** ${mdEscape(project.link_urls.wiki?.url ?? '')}`, 520)
			.shown(({ project }) => !!project.link_urls.wiki?.url),

		linkSection('discord', ({ project }) => `**Discord:** ${mdEscape(project.link_urls.discord?.url ?? '')}`, 530)
			.shown(({ project }) => !!project.link_urls.discord?.url),

		linkSection('site', ({ project }) => `**Website:** ${mdEscape(project.link_urls.site?.url ?? '')}`, 540)
			.shown(({ project }) => !!project.link_urls.site?.url),

		linkSection('store', ({ project }) => `**Store:** ${mdEscape(project.link_urls.store?.url ?? '')}`, 550)
	...
	],
	{
		...
		shown: (_project, projectV3) =>
			Boolean(
				projectV3 && Object.keys(projectV3.link_urls).length > 0,
			),
	},
)
