...
export function deserializeActionStates(data: string): Record<string, ActionState> {
    try {
        ...
        for (const [key, state] of Object.entries(
            parsed as Record<string, SerializedActionState>,
        )) {
            states[key] = {
                selected: state.selected,
...
export function flattenProjectVariables(project: Project): Record<string, string> {
    ...
    vars[`PROJECT_SETTINGS_FLINK`] =
        `[Settings](https://modrinth.com/project/${project.id}/settings)`
    ...
    vars[`PROJECT_VERSIONS_FLINK`] =
        `[Versions](https://modrinth.com/project/${project.id}/versions)`
