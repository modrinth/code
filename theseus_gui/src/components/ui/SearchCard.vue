  ...
  <Card class="card button-base" @click="$router.push(`/project/${project.project_id}`)">
    <div class="card-content">
      ...
      <div class="content badges">
        <Button :to="`/browse/${project.slug}`" color="primary" :disabled="installed || installing" @click.stop="install()">
...
import { Avatar, Card, Categories, EnvironmentIndicator, Button, DownloadIcon, formatNumber, formatCategory, HeartIcon, CalendarIcon, CheckIcon } from "omorphia";
...
const installed = ref(props.instance && Object.values(props.instance.projects).some(project => project?.metadata?.project?.id === props.project.project_id))
const installing = ref(false)

const install = async () => {
  installing.value = true
  const versions = await ofetch(`https://api.modrinth.com/v2/project/${props.project.project_id}/version`)
  let queuedVersionData

  if (!props.instance) {
    queuedVersionData = versions[0]
  } else {
    queuedVersionData = versions.find((v) =>
      v.game_versions.includes(props.instance.game_version)
    )
  }
