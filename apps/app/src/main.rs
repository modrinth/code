				...
				.download_and_install(
					|chunk_length, content_length| {
						let _ = theseus::emit_loading(
							&loader_bar_id,
							(chunk_length as f64)
								/ (content_length.unwrap_or(DEFAULT_CONTENT_LENGTH) as f64),
							None,
						);
					},
					|| {},
		...
		.setup(|app| {
			#[cfg(target_os = "macos")]
			{
				...
				app.listen("deep-link://new-url", move |url| {
					...
					let actual_request = serde_json::from_str::<Vec<String>>(&request)
						.ok()
		...
		Ok(app) => {
			app.run(|app, event| {
				...
				if let tauri::RunEvent::Opened { urls } = event {
					...
					let file = urls.into_iter().find_map(|url| url.to_file_path().ok());
