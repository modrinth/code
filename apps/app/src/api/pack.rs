...
#[tauri::command]
pub async fn pack_install(location: CreatePackLocation, profile: String) -> Result<String> {
	Ok(install_zipped_mrpack(location, profile).await?)
}

#[tauri::command]
pub fn pack_get_profile_from_pack(location: CreatePackLocation) -> Result<CreatePackProfile> {
