...
#[tauri::command]
pub async fn progress_bars_list() -> Result<DashMap<uuid::Uuid, theseus::LoadingBar>> {
...
pub async fn should_disable_mouseover() -> bool {
	if cfg!(target_os = "macos") {
		...
		if let tauri_plugin_os::Version::Semantic(major, minor, _) = tauri_plugin_os::version()
			&& major >= 12
			&& minor >= 3
		{
...
#[tauri::command]
pub fn highlight_in_folder<R: Runtime>(app: tauri::AppHandle<R>, path: PathBuf) {
...
pub(crate) fn tauri_convert_file_src(path: &Path) -> Result<Url> {
	...
	macro_rules! theseus_try {
		($test:expr) => {
			match $test {
				Ok(val) => val,
				Err(e) => return Err(TheseusSerializableError::Theseus(e.into())),
			}
		};
	}
