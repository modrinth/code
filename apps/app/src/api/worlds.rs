...
use theseus::worlds::{
	DisplayStatus, ProtocolVersion, ServerPackStatus, ServerStatus, World, WorldType,
	WorldWithProfile,
};
...
pub async fn get_recent_worlds<R: Runtime>(
	...
	let mut result =
		worlds::get_recent_worlds(limit, display_statuses.unwrap_or(EnumSet::all())).await?;
...
fn adapt_world_icon<R: Runtime>(app_handle: &AppHandle<R>, world: &mut World) {
	if let Some(Either::Left(icon_path)) = &world.icon {
		...
		if let Ok(new_url) = super::utils::tauri_convert_file_src(&icon_path) {
			...
			if let Err(e) = app_handle.asset_protocol_scope().allow_file(&icon_path) {
				tracing::warn!(
...
pub async fn set_world_display_status(
	...
	Ok(worlds::set_world_display_status(instance, world_type, world_id, display_status).await?)
}

#[tauri::command]
pub async fn rename_world(instance: &str, world: &str, new_name: &str) -> Result<()> {
...
pub async fn add_server_to_profile(
	...
	Ok(worlds::add_server_to_profile(&path, name, address, pack_status).await?)
}
...
pub async fn edit_server_in_profile(
	...
	let path = get_full_path(path).await?;
	worlds::edit_server_in_profile(&path, index, name, address, pack_status).await?;
	Ok(())
}

#[tauri::command]
pub async fn remove_server_from_profile(path: &str, index: usize) -> Result<()> {
	let path = get_full_path(path).await?;
...
#[tauri::command]
pub async fn get_profile_protocol_version(path: &str) -> Result<Option<ProtocolVersion>> {
...
#[tauri::command]
pub async fn start_join_singleplayer_world(path: &str, world: String) -> Result<ProcessMetadata> {
	let process = profile::run(path, QuickPlayType::Singleplayer(world)).await?;

	Ok(process)
}

#[tauri::command]
pub async fn start_join_server(path: &str, address: &str) -> Result<ProcessMetadata> {
