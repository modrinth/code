...
pub async fn login<R: Runtime>(app: tauri::AppHandle<R>) -> Result<Option<Credentials>> {
	...
	let window = tauri::WebviewWindowBuilder::new(
		&app,
		"signin",
		tauri::WebviewUrl::External(flow.auth_request_uri.parse().map_err(|_| {
			theseus::ErrorKind::OtherError("Error parsing auth redirect URL".to_string()).as_error()
		})?),
	)
	...
	while (Utc::now() - start) < Duration::minutes(10) {
		...
		if window
			...
			&& let Some((_, code)) = window.url()?.query_pairs().find(|x| x.0 == "code")
		{
			window.close()?;
