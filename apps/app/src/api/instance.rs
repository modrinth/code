...
pub async fn instance_get(instance_id: &str) -> Result<Option<Instance>> {
	Ok(theseus::instance::get(instance_id).await?.map(Instance::from))
}

#[tauri::command]
pub async fn instance_get_many(instance_ids: Vec<String>) -> Result<Vec<Instance>> {
...
pub async fn instance_get_content_items(
	...
	Ok(theseus::instance::get_content_items(instance_id, cache_behaviour).await?)
}
...
pub async fn instance_get_linked_modpack_info(
	...
	Ok(theseus::instance::get_linked_modpack_info(instance_id, cache_behaviour)
		.await?)
}
...
pub async fn instance_check_installed(
	...
	if let Ok(projects) = theseus::instance::get_projects(instance_id, None).await {
...
#[tauri::command]
pub async fn instance_update_all(instance_id: &str) -> Result<HashMap<String, String>> {
...
pub async fn instance_update_project(
	...
	Ok(theseus::instance::update_project(instance_id, project_path, None).await?)
}
...
pub async fn instance_toggle_disable_project(
	...
	Ok(theseus::instance::toggle_disable_project(instance_id, project_path).await?)
}
