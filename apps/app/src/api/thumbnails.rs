...
pub async fn get_image_thumbnail<R: Runtime>(
	...
	let cache = theseus::State::get().await?.directories.caches_dir().join("thumbnails-v1");
	let permit = THUMBNAIL_WORKER.acquire().await.map_err(std::io::Error::other)?;
...
fn create_thumbnail(path: &Path, cache: &Path, size: u32) -> std::io::Result<PathBuf> {
	...
	thumbnail.save_with_format(&temporary, image::ImageFormat::Png).map_err(std::io::Error::other)?;
...
fn prune_cache(cache: &Path, current: &Path) -> std::io::Result<()> {
	...
	for entry in std::fs::read_dir(cache)? {
		...
		total += metadata.len();
		entries.push((metadata.modified().unwrap_or(UNIX_EPOCH), metadata.len(), entry.path()));
	}
	entries.sort_unstable_by_key(|(modified, _, _)| *modified);
	for (_, size, path) in entries {
		if total <= CACHE_LIMIT { break; }
		if path != current && std::fs::remove_file(path).is_ok() { total -= size; }
	}
	Ok(())
}
