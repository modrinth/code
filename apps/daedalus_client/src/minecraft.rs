...
use crate::{
	Error, MirrorArtifact, UploadFile, util::download_file, util::format_url, util::sha1_async,
};
use daedalus::minecraft::{
	Library, PartialLibrary, VERSION_MANIFEST_URL, VersionInfo, VersionManifest,
...
pub async fn fetch(
	...
	let (fetch_versions, existing_versions) = if let Some(mut modrinth_manifest) = modrinth_manifest
	{
		...
		for version in mojang_manifest.versions {
			...
			{
				let modrinth_version = modrinth_manifest.versions.remove(index);
