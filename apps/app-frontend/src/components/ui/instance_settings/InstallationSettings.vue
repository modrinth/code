    ...
    get_loaders()
        .then((value: PlatformTag[]) =>
            value
                .filter(
                    (item) =>
                        item.supported_project_types.includes('modpack') || item.name === 'vanilla',
                )
                ...
                get_version_many(project.versions, 'must_revalidate')
                    .then((versions: Version[]) => {
                        ...
                        modpackVersion.value =
                            versions.find(
                                (version: Version) =>
                                    version.id === props.instance.linked_data?.version_id,
                            ) ?? null
                    })
                    .catch(handleError)
                    .finally(() => {
...
const selectableLoaderVersions: ComputedRef<ManifestLoaderVersion[] | undefined> = computed(() => {
    if (gameVersion.value) {
        if (loader.value === 'fabric') {
            ...
            return forge_versions?.value?.gameVersions?.find(
                (item) => item.id === gameVersion.value,
            )?.loaders
        } else if (loader.value === 'quilt') {
            return quilt_versions?.value.gameVersions[0].loaders
        } else if (loader.value === 'neoforge') {
            return neoforge_versions?.value?.gameVersions?.find(
                (item) => item.id === gameVersion.value,
            )?.loaders
        }
    }
...
const messages = defineMessages({
    ...
    noLoaderVersions: {
        ...
        defaultMessage:
            '{loader} is not available for Minecraft {version}. Try another mod loader.',
    },
    noConnection: {
        id: 'instance.settings.tabs.installation.no-connection',
        defaultMessage:
            'Cannot fetch linked modpack details. Please check your internet connection.',
    },
    noModpackFound: {
        ...
        <div class="flex gap-4 items-center justify-between p-4 bg-bg rounded-2xl">
            ...
            <template v-else>
                <div class="flex gap-2 items-center">
                    ...
                    >
                        <div
                            v-if="!!currentLoaderIcon"
                            class="contents"
                            v-html="currentLoaderIcon"
                        />
                        <WrenchIcon v-else />
                    </div>
