...
const isProfileCompatible = (profile) =>
  versions.value?.some((version) =>
    isVersionCompatible(version, project.value, profile),
  )
...
defineExpose({
  show: async (projectVal, versionsVal, callback) => {
    ...
    for (const profile of profilesVal) {
      ...
      profile.installedMod = await check_installed(
        profile.path,
        project.value.id,
      ).catch(handleError)
    }
...
const createInstance = async () => {
  ...
  console.log('CREATE INSTANCE!', versions.value[0])
  console.log('CREATE INSTANCE loaders', versions.value[0].loaders)
  console.log(
    'CREATE INSTANCE loaders contains',
    versions.value[0].loaders.contains,
  )
  ...
  await installVersionDependencies(instance, versions.value[0]).catch(
    handleError,
  )
      ...
      <div class="profiles" :class="{ 'hide-creation': !showCreation }">
        ...
        >
          ...
          >
            <Avatar
              :src="
                profile.icon_path ? convertFileSrc(profile.icon_path) : null
              "
              class="profile-image"
            />
            {{ profile.name }}
      ...
      <Card v-if="showCreation" class="creation-card">
        <div class="creation-container">
          ...
          <div class="creation-settings">
            ...
            />
            <Button
              :disabled="creatingInstance === true || !name"
