    ...
    <div v-if="worlds.length > 0" class="flex flex-col gap-4">
        ...
        <div class="flex flex-col w-full gap-2">
            <WorldItem
                ...
                :refreshing="
                    world.type === 'server' ? serverData[world.address]?.refreshing : undefined
                "
                :server-status="
                    world.type === 'server' ? serverData[world.address]?.status : undefined
                "
                :rendered-motd="
                ...
                @open-folder="
                    (world: SingleplayerWorld) => showWorldInFolder(instance.path, world.path)
                "
            />
        </div>
    </div>
...
const filteredWorlds = computed(() =>
    worlds.value.filter((x) => {
        ...
        const typeFilter =
            filters.value.includes('server') || filters.value.includes('singleplayer')
