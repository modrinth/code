								...
								<template v-if="isOwner" #actions>
									<div class="absolute right-2 top-2 z-10">
										<ButtonStyled
											circular
											type="transparent"
											class="!bg-surface-2/50 backdrop-blur-sm shadow-sm hover:!bg-surface-2"
										>
											<button
												v-tooltip="
											...
											>
												<StarIcon
													class="h-5 w-5"
													:class="{
														'text-yellow-400 fill-current': isFeatured(project.id),
...
const isOwner = computed(() => !!auth.value?.user && !!user.value && auth.value.user.id === user.value.id)
