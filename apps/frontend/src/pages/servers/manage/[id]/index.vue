<template>
    <div class="relative flex select-none flex-col gap-6" data-pyro-server-manager-root>
            ...
            <div class="flex w-full justify-between gap-2">
                <div v-if="inspectingError.analysis.problems.length" class="flex flex-row gap-4">
                    ...
                    <div class="flex flex-col gap-2">
                        <div class="font-semibold">
                            {{ serverData?.name }} shut down unexpectedly. We've automatically
                            analyzed the logs and found the following problems:
                        </div>
                        ...
                        >
                            ...
                            <ul class="m-0 ml-6">
                                <li v-for="solution in problem.solutions" :key="solution.message">
                                    <span class="m-0 text-sm font-normal">{{
                                        solution.message
                                    }}</span>
                                </li>
                            </ul>
                    ...
                    <div class="flex flex-col gap-2">
                        ...
                        <div class="font-normal">
                            <template v-if="props.powerStateDetails?.oom_killed">
                                The server stopped because it ran out of memory. There may be a
                                memory leak caused by a mod or plugin, or you may need to upgrade
                                your Modrinth Server.
                            </template>
                            <template v-else-if="props.powerStateDetails?.exit_code !== undefined">
                                We could not automatically determine the specific cause of the
                                crash, but your server exited with code
                ...
                <div v-else class="flex flex-row gap-4">
                    ...
                    <div class="flex flex-col gap-2">
                        ...
                        <div class="font-normal">
                            We could not find any specific problems, but you can try restarting the
                            server.
                        </div>
                    </div>
                </div>
        ...
        >
            ...
            <p>
                An error occurred while attempting to connect to your server. Please try refreshing
                the page. (WebSocket Authentication Failed)
            </p>
        </div>
    </div>
</template>
...
const acceptSuggestion = () => {
    ...
    if (
        currentTokens[currentTokens.length - 1].toLowerCase() === suggestionTokens[0].toLowerCase()
    ) {
        /* empty */
    } else {
        const offset =
