  ...
  <div class="contents">
    <NewModal ref="newAllocationModal" header="New allocation">
      <form class="flex flex-col gap-2 md:w-[600px]" @submit.prevent="addNewAllocation">
        <label for="new-allocation-name" class="text-contrast font-semibold"> Name </label>
        <input
        ...
        />
        <div class="mt-4 mb-1 flex justify-start gap-4">
    ...
    <NewModal ref="editAllocationModal" header="Edit allocation">
      <form class="flex flex-col gap-2 md:w-[600px]" @submit.prevent="editAllocation">
        <label for="edit-allocation-name" class="text-contrast font-semibold"> Name </label>
        <input
          ...
          <div class="card flex flex-col gap-4">
            <div class="flex w-full flex-col items-center justify-between gap-4 sm:flex-row">
              <label for="user-domain" class="flex flex-col gap-2">
                <span class="text-contrast text-lg font-bold">Generated DNS records</span>
                <span>
            ...
            <div
              class="bg-table-alternate-row flex max-w-full flex-none overflow-auto rounded-xl px-4 py-2"
            >
              <table
                  ...
                  <tr v-for="record in dnsRecords" :key="record.content" class="w-full">
                    <td class="w-1/6 py-3 pr-4 md:w-1/5 md:pr-8 lg:w-1/4 lg:pr-12">
                      <div class="flex flex-col gap-1" @click="copyText(record.type)">
                        <span
                          class="text-md text-contrast font-bold tracking-wide hover:cursor-pointer"
                        >
                          {{ record.type }}
                        </span>
                        <span class="text-secondary text-xs">Type</span>
                      </div>
                    </td>
                    <td class="w-2/6 py-3 md:w-1/3">
                      <div class="flex flex-col gap-1" @click="copyText(record.name)">
                        <span
                          class="text-md text-contrast truncate font-bold tracking-wide hover:cursor-pointer"
                        >
                          {{ record.name }}
                        </span>
                        <span class="text-secondary text-xs">Name</span>
                      </div>
                    </td>
                    <td class="w-3/6 py-3 pl-4 md:w-5/12 lg:w-5/12">
                      <div class="flex flex-col gap-1" @click="copyText(record.content)">
                        <span
                          class="text-md text-contrast w-fit truncate font-bold tracking-wide hover:cursor-pointer"
                        >
                          {{ record.content }}
                        </span>
                        <span class="text-secondary text-xs">Content</span>
                      </div>
                    </td>
                  </tr>
                </tbody>
          ...
          <div class="card flex flex-col gap-4">
            <div class="flex w-full flex-col items-center justify-between gap-4 sm:flex-row">
              <div class="flex flex-col gap-2">
                <span class="text-contrast text-lg font-bold">Allocations</span>
                <span>
            ...
            <div class="bg-table-alternate-row flex w-full flex-col overflow-hidden rounded-xl p-4">
              <!-- Primary allocation -->
              <div class="flex flex-col justify-between gap-2 sm:flex-row sm:items-center">
                <span class="text-md text-contrast font-bold tracking-wide">
                  Primary allocation
                </span>
            ...
            <div
              v-if="allocations?.[0]"
              class="bg-table-alternate-row flex w-full flex-col gap-4 overflow-hidden rounded-xl p-4"
            >
              <div
                ...
                <div class="flex flex-row items-center gap-4">
                  ...
                  <div class="flex w-[20rem] flex-col justify-between sm:flex-row sm:items-center">
                    <div class="flex flex-col gap-1">
                      ...
                      <span class="text-secondary hidden text-xs sm:block">Name</span>
                    </div>
                    <div class="flex flex-col gap-1">
                      <span
                        class="text-md text-secondary sm:text-contrast w-10 tracking-wide sm:font-bold"
                      >
                        {{ allocation.port }}
                      </span>
                      <span class="text-secondary hidden text-xs sm:block">Port</span>
                    </div>
                  </div>
                </div>
