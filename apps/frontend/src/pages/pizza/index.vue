<template>
    <NewModal ref="modal" header="Build your pizza">
      <div class="flex flex-col gap-4">
        ...
        <div class="flex flex-col gap-2">
          ...
          <span>Your preselected topping choice. Our advanced AI determined these top 3 toppings for you.</span>
          <div>
            {{ toppingOptions.join(', ') }}
          </div>
        </div>
        ...
        <div class="flex flex-col gap-2" v-if="toppingOptions.includes('Pineapple Ore')">
          <span class="text-lg font-semibold text-contrast">Order validation</span>
          <div class="flex flex-row gap-2">
            <span class="text-yellow-500"><b>Warning!</b> Pineapple detected. This may cause unknown side effects.</span>
          </div>
        </div>
        ...
        <div v-else class="flex flex-col gap-2">
          <span class="text-lg font-semibold text-contrast">Order validation</span>
          <span class="text-green-500"><b>Success!</b> No pineapple detected. Your order is safe.</span>
        </div>
  ...
  <div v-if="isPreapring" class="fixed top-0 left-0 w-full h-full bg-black bg-opacity-50 flex items-center justify-center">
    <div class="bg-zinc-800 p-8 rounded-lg shadow-lg">
      <h2 class="text-2xl font-semibold mb-4">Your pizza is being prepared...</h2>
      <p class="text-lg">Your chef, <span class="font-semibold">{{ whoIsPreparing }}</span>, is preparing your pizza. Please wait warmly.</p>
      <p class="text-lg mt-2">Estimated time left: <span class="font-semibold">{{ preparingTime }} seconds</span></p>
    </div>
  </div>

  <main class="flex-1 flex flex-col items-center justify-center text-center px-6 py-16">
    <img class="w-48 md:w-64 h-auto mb-4" src="https://i.hep.gg/RRLj-WEql" alt="Modrinth Pizza">

    <h2 class="text-4xl md:text-5xl font-bold mb-4">
      Welcome to Modrinth Pizza
    </h2>
    <p class="text-lg max-w-xl mb-6">
      Say hello to <span class="text-green-400 font-semibold">Modrinth Pizza</span> — the world's first modded pizza delivery service.
      Pick your loader (Forge, Fabric, or Quilt crust), and get delicious slices shipped straight to your server.
    </p>

    <p class="text-base max-w-lg mb-8">
      Compatible with all Minecraft flavors, auto-updates via <code>cheese.toml</code>, and is <span class="text-green-400 font-semibold">fully open sauce</span>.
    </p>
    ...
    <p class="text-sm text-gray-500 mt-4 max-w-lg">
      Disclaimer: Modrinth Pizza is not responsible for any mod conflicts, crashes, or server outages caused by our pizza. Eat at your own risk.
      By ordering, you agree to this and to allow us to collect your data to sell to the highest bidder. Bon appétit! 🍕
    </p>
  </main>
</template>
...
const whoIsPreparing = ref(modrinthTeamMembers[Math.floor(Math.random() * modrinthTeamMembers.length)]);
