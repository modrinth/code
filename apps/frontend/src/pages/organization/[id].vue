                ...
                <template v-if="projects && projects.length > 0">
                    <div class="project-list display-mode--list">
                        <ProjectCard
                                ...
                                ? (projects ?? []).filter((x) =>
                                      x.project_types.includes(
                                          typeof route.params.projectType === 'string'
                                              ? route.params.projectType.slice(
                                                    0,
                                                    route.params.projectType.length - 1,
                                                )
                            ...
                            :featured-image="
                                project.gallery.find((element) => element.featured)?.url
                            "
                ...
                <div v-else-if="true" class="error">
                    ...
                    <span class="preserve-lines text">
                        ...
                        <template v-if="isPermission(currentMember?.permissions, 1 << 4)">
                            Would you like to
                            <a class="link" @click="($refs as any).modal_creation?.show()"
                                >create one</a
                            >?
                        </template>
                    </span>
...
const [
    ...
    useAsyncData(
        ...
        () =>
            useBaseFetch(`organization/${orgId}/projects`, { apiVersion: 3 }) as Promise<
                ProjectV3[]
            >,
        {
            transform: (projects) => {
                for (const project of projects) {
                    ...
                    const clientAndServer =
                        project.client_and_server && project.client_and_server[0]
