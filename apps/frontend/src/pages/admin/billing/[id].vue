        ...
        <div class="flex flex-col gap-3">
            ...
            <div class="flex flex-col gap-2">
                <label for="unprovision" class="flex flex-col gap-1">
                    ...
                    <span>
                        Whether or not the subscription should be unprovisioned on refund.
                    </span>
                </label>
        ...
        <div class="flex flex-col gap-3">
            <div class="flex flex-col gap-2">
                <label for="cancel" class="flex flex-col gap-1">
                    ...
                    <span>
                        Whether or not the subscription should be cancelled. Submitting this as
                        "true" will cancel the subscription, while submitting it as "false" will
            ...
            <div v-for="subscription in subscriptionCharges" :key="subscription.id" class="card">
                <div class="mb-4 grid grid-cols-[1fr_auto]">
                    <div>
                        ...
                        <div
                            class="mb-4 mt-2 flex w-full items-center gap-1 text-sm text-secondary"
                        >
                            {{ formatCategory(subscription.interval) }} ⋅
                            {{ subscription.status }} ⋅
                    ...
                    >
                        <div
                            ...
                            :class="
                                charge.type === 'refund'
                                    ? 'bg-purple'
                                    : chargeStatuses[charge.status].color
                            "
                        />
                            ...
                            <div class="flex flex-col gap-2">
                                <span>
                                    <span class="font-bold text-contrast">
                                        <template v-if="charge.status === 'succeeded'">
                                            Succeeded
                                        </template>
                                        <template v-else-if="charge.status === 'failed'">
                                            Failed
                                        </template>
                                        <template v-else-if="charge.status === 'cancelled'">
                                            Cancelled
                                        </template>
                                        <template v-else-if="charge.status === 'processing'">
                                            Processing
                                        </template>
                                        <template v-else-if="charge.status === 'open'">
                                            Upcoming
                                        </template>
                                    ...
                                    ⋅
                                    <span>
                                        <template v-if="charge.type === 'refund'">
                                            Refund
                                        </template>
                                        <template v-else-if="charge.type === 'subscription'">
                                            ...
                                            <template
                                                v-else-if="
                                                    index === subscription.charges.length - 1
                                                "
                                            >
                                        ...
                                        <template v-else-if="charge.type === 'one-time'">
                                            One-time charge
                                        </template>
                                        <template v-else-if="charge.type === 'proration'">
                                            Proration charge
                                        </template>
                                        <template v-else> {{ charge.status }} </template>
                                    </span>
                                ...
                                <span class="text-sm text-secondary">
                                    <span
                                        v-if="
                                            ...
                                            $dayjs(charge.due).isBefore($dayjs())
                                        "
                                        class="font-bold"
                                    >
                                        Ended:
                                    ...
                                    <span
                                        v-else-if="charge.status === 'cancelled'"
                                        class="font-bold"
                                        >Ends:</span
                                    >
                                    <span v-else-if="charge.type === 'refund'" class="font-bold"
                                        >Issued:</span
                                    >
                                ...
                                <span
                                    v-if="charge.last_attempt != null"
                                    class="text-sm text-secondary"
                                >
                                    <span v-if="charge.status === 'failed'" class="font-bold"
                                        >Last attempt:</span
                                    >
                                ...
                                <ButtonStyled
                                    v-if="
                                        charges.some(
                                            (x) =>
                                                x.type === 'refund' &&
                                                x.parent_charge_id === charge.id,
                                        )
                                    "
                                >
                                    <div class="button-like disabled">
                                        <CheckIcon /> Charge refunded
                                    </div>
                                </ButtonStyled>
                                <ButtonStyled
                                    v-else-if="
                                        charge.status === 'succeeded' && charge.type !== 'refund'
                                    "
                                    color="red"
                                    color-fill="text"
                                >
