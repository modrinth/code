    ...
    <div v-else class="experimental-styles-within">
                ...
                <div class="mx-auto flex max-w-[40rem] flex-col gap-4 md:w-[30rem]">
                    <div
                        v-if="
                            ...
                            project.loaders.some(
                                (x) => !tags.loaderData.allPluginLoaders.includes(x),
                            )
                        "
                    ...
                    <div class="mx-auto flex w-fit flex-col gap-2">
                        <ButtonStyled v-if="project.game_versions.length === 1">
                            <div class="disabled button-like">
                                ...
                                <InfoIcon
                                    v-tooltip="
                                        `${project.title} is only available for ${currentGameVersion}`
                                    "
        ...
        >
                    ...
                    <template #actions>
                        <div class="hidden sm:contents">
                            <ButtonStyled
                                size="large"
                                :color="
                                    route.name === 'type-id-version-version' ? `standard` : `brand`
                                "
                            >
                        ...
                        >
                            ...
                            <template #popper>
                                <div
                                    class="experimental-styles-within flex max-w-60 flex-col gap-1"
                                >
                                    <div class="flex items-center justify-between gap-4">
                                        <h3
                                            class="m-0 flex items-center gap-2 text-base font-bold text-contrast"
                                        >
                                            Create a server
                                            <TagItem
                                                :style="{
                                    ...
                                    >
                                        Modrinth Servers is the easiest way to play with your
                                ...
                                >
                                    ...
                                    <template #menu>
                                        ...
                                        <div
                                            ...
                                            class="collections-list text-primary"
                                        >
                                            <Checkbox
                                                ...
                                                @update:model-value="
                                                    () => onUserCollectProject(option, project.id)
                                                "
                                            >
                                                {{ option.name }}
                                            </Checkbox>
                        ...
                        <ButtonStyled size="large" circular type="transparent">
                            ...
                            >
                                ...
                                <template #analytics>
                                    <ChartIcon aria-hidden="true" /> Analytics
                                </template>
                                <template #moderation-checklist>
                                    <ScaleIcon aria-hidden="true" /> Review project
                                </template>
                                <template #report>
                                    <ReportIcon aria-hidden="true" /> Report
                                </template>
                                <template #copy-id>
                                    <ClipboardCopyIcon aria-hidden="true" /> Copy ID
                                </template>
            ...
            <div class="normal-page__sidebar">
                ...
                />
                <AdPlaceholder
                    v-if="!auth.user && tags.approvedStatuses.includes(project.status)"
                />
                <ProjectSidebarLinks
                    :project="project"
...
const possiblePlatforms = computed(() => {
    return versions.value
        .filter(
            (x) => !currentGameVersion.value || x.game_versions.includes(currentGameVersion.value),
        )
        .flatMap((x) => x.loaders)
})
...
const currentPlatform = computed(() => {
    return (
        ...
        (project.value.loaders.length === 1 && project.value.loaders[0])
    )
})
...
function handleError(err, project = false) {
    if (err.value && err.value.statusCode) {
        throw createError({
            ...
            message:
                err.value.statusCode === 404 && project ? 'Project not found' : err.value.message,
        })
    }
}
