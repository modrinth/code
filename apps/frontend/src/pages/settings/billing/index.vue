                ...
                <div class="flex w-full flex-wrap justify-between gap-4 xl:w-auto xl:flex-col">
                    <div class="flex flex-col gap-1 xl:ml-auto xl:text-right">
                        <span class="text-2xl font-bold text-dark">
                            <template v-if="midasCharge">
                                {{
                                    formatPrice(
                                        vintl.locale,
                                        midasSubscriptionPrice.prices.intervals[
                        ...
                        <template v-if="midasCharge">
                            ...
                            <span
                                v-if="midasCharge.status === 'open'"
                                class="text-sm text-secondary"
                            >
                                Renews {{ $dayjs(midasCharge.due).format('MMMM D, YYYY') }}
                            </span>
                            <span
                                v-else-if="midasCharge.status === 'cancelled'"
                                class="text-sm text-secondary"
                            >
                    ...
                    <div class="flex flex-col gap-4">
                        ...
                        <div class="flex flex-col gap-2">
                            ...
                            <div v-else class="w-fit">
                                <p>
                                    A linked server couldn't be found for this subscription. There
                                    are a few possible explanations for this. If you just purchased
                                    your server, this is normal. It could take up to an hour for
                                    your server to be provisioned. Otherwise, if you purchased this
                                    server a while ago, it has likely since been suspended. If this
                                    is not what you were expecting, please contact Modrinth Support
                                ...
                                <div class="flex w-full flex-col gap-2">
                                    ...
                                    <CopyCode
                                        class="whitespace-nowrap"
                                        :text="'Stripe ID: ' + subscription.id"
                                    />
                                </div>
                            </div>
                            ...
                            <div class="flex flex-row justify-between">
                                        ...
                                        <span>
                                            {{
                                                getPyroProduct(subscription)?.metadata?.storage
                                                    ? getPyroProduct(subscription).metadata
                                                          .storage /
                                                          1024 +
                                                      ' GB SSD'
                                                    : ''
                                ...
                                <div class="flex flex-col items-end justify-between">
                                    <div class="flex flex-col items-end gap-2">
                                        ...
                                        <div
                                            v-if="getPyroCharge(subscription)"
                                        ...
                                        >
                                            ...
                                            >
                                                Renews
                                                {{
                                                    $dayjs(getPyroCharge(subscription).due).format(
                                                        'MMMM D, YYYY',
                                                    )
                                                }}
                                            </span>
                                            <span
                                                v-else-if="
                                                    getPyroCharge(subscription).status ===
                                                    'processing'
                                                "
                                                class="text-sm text-orange"
                                            >
                                                Your payment is being processed. Your server will
                                                activate once payment is complete.
                                            </span>
                                            <span
                                                v-else-if="
                                                    getPyroCharge(subscription).status ===
                                            ...
                                            >
                                                Your subscription payment failed. Please update your
                                                payment method, then resubscribe.
                                            </span>
                                        </div>
                                    </div>
                                            ...
                                            <button
                                                @click="
                                                    resubscribePyro(
                                                        subscription.id,
                                                        $dayjs(
                                                            getPyroCharge(subscription).due,
                                                        ).isBefore($dayjs()),
                                                    )
                                                "
                                            >
                                ...
                                <template v-if="method.type === 'card'">
                                    {{
                                        formatMessage(messages.paymentMethodCardDisplay, {
                                            card_brand:
                                                formatMessage(
                                                    paymentMethodTypes[method.card.brand],
...
const midasSubscription = computed(() =>
    subscriptions.value?.find(
        (x) =>
            ...
            midasProduct.value?.prices?.find((y) => y.id === x.price_id),
    ),
)
