<template>
    <section class="universal-card">
        ...
        <h3>Amount</h3>
        <p>
            You are initiating a transfer of your revenue from Modrinth's Creator Monetization
            Program. How much of your
            <strong>{{ $formatMoney(userBalance.available) }}</strong> balance would you like to
            transfer transfer to {{ selectedMethod.name }}?
        </p>
            ...
            <template v-else>
                <div>
                    <p>
                        ...
                        <strong>{{ $formatMoney(minWithdrawAmount) }}</strong> and a maximum
                        transfer amount of <strong>{{ $formatMoney(maxWithdrawAmount) }}</strong
                        >.
                    </p>
                    <input
                        id="confirmation"
                    ...
                    />
                    <p>
                        You have entered <strong>{{ $formatMoney(parsedAmount) }}</strong> to
                        transfer.
                    </p>
                </div>
            </template>
            ...
            <template v-if="knownErrors.length === 0 && amount">
                ...
                <Checkbox v-model="agreedTransfer" description="Confirm transfer">
                    <template v-if="selectedMethod.type === 'tremendous'">
                        I confirm that I am initiating a transfer and I will receive further
                        instructions on how to redeem this payment via email to:
                        {{ withdrawAccount }}
                    </template>
                    <template v-else>
