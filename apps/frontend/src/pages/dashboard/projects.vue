    ...
    <div>
        <Modal ref="editLinksModal" header="Edit links">
            <div class="universal-modal links-modal">
                <p>
                    Any links you specify below will be overwritten on each of the selected
                    projects. Any you leave blank will be ignored. You can clear a link from all
                    selected projects using the trash can button.
                </p>
                <section class="links">
                    <label
                    ...
                    <div class="push-right">
                        <div class="labeled-control-row">
                            ...
                            <Multiselect
                                ...
                                :allow-empty="false"
                                @update:model-value="
                                    projects = updateSort(projects, sortBy, descending)
                                "
                            />
                            <button
...
export default defineNuxtComponent({
    ...
    methods: {
        ...
        async bulkEditLinks() {
            try {
                const baseData = {
                    issues_url: this.editLinks.issues.clear
                        ? null
                        : this.editLinks.issues.val.trim(),
                    source_url: this.editLinks.source.clear
                        ? null
                        : this.editLinks.source.val.trim(),
                    wiki_url: this.editLinks.wiki.clear ? null : this.editLinks.wiki.val.trim(),
                    discord_url: this.editLinks.discord.clear
                        ? null
                        : this.editLinks.discord.val.trim(),
                }

                if (!baseData.issues_url?.length || baseData.issues_url.length === 0) {
                    delete baseData.issues_url
                }

                if (!baseData.source_url?.length || baseData.source_url.length === 0) {
                    delete baseData.source_url
                }

                if (!baseData.wiki_url?.length || baseData.wiki_url.length === 0) {
                    delete baseData.wiki_url
                }

                if (!baseData.discord_url?.length || baseData.discord_url.length === 0) {
