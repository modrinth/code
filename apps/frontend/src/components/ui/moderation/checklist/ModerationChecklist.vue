        ...
        <div class="flex grow-0 items-center gap-2">
            ...
            <ButtonStyled circular>
                <a
                    ...
                    target="_blank"
                    :href="currentStageObj.guidance_url"
                >
                    <FileTextIcon />
                </a>
            </ButtonStyled>
                ...
                <div v-else>
                    ...
                    <div class="space-y-4">
                        ...
                        <div
                            ...
                            class="dropdown-actions-group space-y-3"
                        >
                                ...
                                <div class="inputs universal-labels">
                                    <div>
                                        ...
                                        <DropdownSelect
                                            ...
                                            :disabled="false"
                                            :display-name="
                                                (opt: any) => opt?.label || 'Unknown option'
                                            "
                                            @update:model-value="
                                                (selected: any) =>
                                                    selectDropdownOption(action, selected)
                                            "
                                        />
                                    </div>
                                </div>
                                ...
                                <div>
                                    ...
                                    <div class="flex flex-wrap gap-2">
                                        <ButtonStyled
                                            v-for="(
                                                option, optIndex
                                            ) in getVisibleMultiSelectOptions(action)"
                                            :key="`${getActionId(action)}-chip-${optIndex}`"
                                            :color="
                                                isChipSelected(action, optIndex)
                                                    ? 'brand'
                                                    : 'standard'
                                            "
                    ...
                    <div class="space-y-4">
                        <template
                            ...
                            :key="`inputs-${getActionKey(action)}`"
                        >
                            <div
                            ...
                            >
                                <div
                                    v-for="(input, inputIndex) in getVisibleInputs(
                                        action,
                                        actionStates,
                                    )"
                                    ...
                                    <template v-if="input.large">
                                        <label :for="`input-${getActionId(action)}-${inputIndex}`">
                                            <span class="label__title">
                                                {{ input.label }}
                                                <span v-if="input.required" class="required"
                                                    >*</span
                                                >
                                            </span>
                                        </label>
                ...
                tryFocusNextAction: () => {
                    ...
                    if (focusedActionIndex.value === null) {
                        ...
                        focusedActionIndex.value =
                            (focusedActionIndex.value + 1) % visibleActions.value.length
                    }
                },
...
function selectDropdownOption(action: DropdownAction, selected: any) {
    ...
    if (state && selected !== undefined && selected !== null) {
        const optionIndex = action.options.findIndex(
            (opt) =>
                opt === selected || (opt?.label && selected?.label && opt.label === selected.label),
        )
...
async function processStageActions(stage: Stage, stageIndex: number, messageParts: MessagePart[]) {
    ...
    for (let actionIndex = 0; actionIndex < stage.actions.length; actionIndex++) {
        ...
        if (action.enablesActions) {
            for (
                let enabledIndex = 0;
                enabledIndex < action.enablesActions.length;
                enabledIndex++
            ) {
...
async function generateMessage() {
    ...
    try {
        ...
        if (props.project.project_type === 'modpack') {
            ...
            if (
                modpackFilesData.interactive.length > 0 ||
                modpackFilesData.permanentNo.length > 0
            ) {
    ...
    const options = checklist
        .map((stage, index) => {
            ...
            return {
                ...
                color:
                    index === currentStage.value && !generatedMessage.value ? 'green' : undefined,
                hoverFilled: true,
