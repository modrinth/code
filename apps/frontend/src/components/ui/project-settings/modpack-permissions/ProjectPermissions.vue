...
const {
	...
	isPending: pending,
} = useQuery(computed(() => projectQueryOptions.attribution(props.project.id, client)))
...
const projectIsApproved = computed(() => props.project.status === 'approved')

const messages = defineMessages({
	noPermissionCount: {
		id: 'project.settings.permissions.no-permission-count',
		defaultMessage: '{count} with no permission',
	},
	userActionCount: {
		id: 'project.settings.permissions.user-action-count',
		defaultMessage: '{count} need user action',
	},
	awaitingReviewCount: {
		id: 'project.settings.permissions.awaiting-review-count',
		defaultMessage: '{count, plural, =0 {None awaiting review} other {# awaiting review}}',
	},
	searchPlaceholder: {
...
const deleteAllGroupsMutation = useMutation({
	mutationFn: () => labrinth.attribution_internal.deleteAllGroups(props.project.id),
	onSuccess: async () => {
		await queryClient.invalidateQueries({ queryKey: ['project-attribution', props.project.id] })
		await props.refreshProjectValidation?.()
		addNotification({
			type: 'success',
