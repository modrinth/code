<template>
	<NewModal ref="taxFormModal" :header="formatMessage(messages.taxFormHeader)"
		...
		<div class="w-full" :class="[
			currentStage === 'form-selection' ? 'sm:w-[540px]' : 'sm:w-[400px]'
		]">
			...
			<div v-else-if="currentStage === 'download-confirmation'" class="flex flex-col gap-6">
				<div class="w-[400px] h-[180px] block rounded-xl relative overflow-hidden rounded-b-none">
					<div class="absolute inset-0 bg-gradient-to-r from-brand-green to-brand-blue rounded-xl rounded-b-none"></div>
					<div class="absolute inset-0 rounded-xl rounded-b-none"
						style="background: linear-gradient(180deg, rgba(39, 41, 46, 0.15) 0%, var(--color-raised-bg) 100%);"></div>
					<svg xmlns="http://www.w3.org/2000/svg" width="220" height="132" viewBox="0 0 220 132" fill="none"
						class="absolute left-[90px] top-[48px] w-[220px] h-[140px]">
				...
				<div class="flex flex-col gap-2">
					<span class="text-2xl text-contrast font-semibold">You're all set! 🎉</span>
					<span>Your {{ determinedFormType }} tax form has been submitted successfully!</span>
					<span>You can freely withdraw now. If you have questions or need to update your details <nuxt-link
							to="https://support.modrinth.com" class="text-link" target="_blank" rel="noopener noreferrer">contact
							support</nuxt-link>.</span>
				</div>
				<div class="w-full flex flex-row justify-stretch gap-2">
					...
					<ButtonStyled color="green">
						<button class="w-full text-contrast" @click="downloadTaxForm">
							<DownloadIcon />Download {{ determinedFormType }}
						</button>
					</ButtonStyled>
				</div>
			</div>
