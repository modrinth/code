<template>
  <div class="flex flex-col gap-4 sm:gap-5">
    ...
    <div v-if="isVenmo" class="flex flex-col gap-2.5">
      <label>
        <span class="text-md font-semibold text-contrast"
          >{{ formatMessage(messages.venmoHandle) }}
          <span class="text-red">*</span></span
        >
      </label>
      <div class="flex flex-row gap-2">
        ...
        <ButtonStyled color="brand">
          <button
            v-tooltip="
              !hasVenmoChanged
                ? 'Change the venmo username to save.'
                : undefined
            "
    ...
    <div class="flex flex-col gap-2.5">
      ...
      <Checkbox v-model="agreedTerms">
        <span>
          <IntlFormatted
            :message-id="financialMessages.rewardsProgramTermsAgreement"
          >
...
const initialVenmoHandle = ref<string>(
  (auth.value.user as any)?.venmo_handle || '',
)
