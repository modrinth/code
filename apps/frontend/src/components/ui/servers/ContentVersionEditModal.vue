    ...
    <NewModal ref="modModal" :header="`Editing ${type.toLocaleLowerCase()} version`">
        <template #title>
            <div class="flex min-w-full items-center gap-2 md:w-[calc(420px-5.5rem)]">
                ...
                <span class="truncate text-xl font-extrabold text-contrast">{{
                    modDetails?.name
                }}</span>
            </div>
        </template>
            ...
            <div class="flex flex-col gap-2">
                <template v-if="versionsLoading">
                    <div class="flex items-center gap-2">
                        <div
                            class="w-fit animate-pulse select-none rounded-md bg-button-bg font-semibold"
                        >
                            <span class="opacity-0" aria-hidden="true">{{ type }} version</span>
                        </div>
                        <div
                            class="min-h-[22px] min-w-[140px] animate-pulse rounded-full bg-button-bg"
                        />
                    </div>
                ...
                <template v-else>
                    ...
                    <UiServersTeleportDropdownMenu
                        ...
                        :display-name="
                            (version) =>
                                typeof version === 'object' ? version?.version_number : version
                        "
                    />
                    <Checkbox v-model="showBetaAlphaReleases">
                        Show any beta and alpha releases
                    </Checkbox>
                </template>
            </div>
                ...
                >
                    <template #platform>
                        ...
                        <svg
                            ...
                            v-html="
                                tags.loaders.find(
                                    (x) => x.name === filtersRef?.selectedPlatforms[0],
                                )?.icon
                            "
                        ></svg>
            ...
            >
                <div>
                    <span>
                        ...
                        {{ type.toLocaleLowerCase() }}. Please try again later or contact support if
                        the issue persists.
                    </span>
    ...
    const versionsWithoutReleaseFilter = currentVersions.value.filter((version: EditVersion) => {
        ...
        return (
            filtersRef.value?.selectedPlatforms.every((platform) =>
                (
                    backwardCompatPlatformMap[
                        platform as keyof typeof backwardCompatPlatformMap
                    ] || [platform]
...
async function show(mod: ContentItem) {
    ...
    try {
        ...
        if (currentModIndex === -1) {
            ...
            currentVersions.value[currentModIndex].version_number =
                `${mod.version_number} (current)`
        ...
        initPlatform.value = platformSet.has(props.loader)
            ...
            : props.loader in backwardCompatPlatformMap
              ? backwardCompatPlatformMap[
                    props.loader as keyof typeof backwardCompatPlatformMap
                ].find((p) => platformSet.has(p)) || defaultPlatform
              : defaultPlatform
