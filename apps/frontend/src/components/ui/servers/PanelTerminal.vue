                        ...
                        >
                            ...
                            >
                                <li
                                    ...
                                    :class="{
                                        'selected-line': isLineSelected(visibleStartIndex + index),
                                    }"
                                    class="group"
                                >
                                ...
                                >
                                    ...
                                    <div class="mx-4 flex flex-row items-center gap-4">
                                        <div class="flex flex-row items-center gap-1">
                                            <div
                                                class="size-1 rounded-full bg-contrast opacity-70"
                                            ></div>
                                            <div
                                                class="size-1 rounded-full bg-contrast opacity-70"
                                            ></div>
                                        ...
                                        <span
                                            class="select-none whitespace-pre text-xs text-contrast"
                                        >
                                            {{
                                                shouldShowSeparator(
                                                    visibleStartIndex + index,
...
const getLineIndexFromEvent = (event: MouseEvent): number | null => {
    ...
    if (searchInput.value) {
        ...
        return Math.max(
            0,
            Math.min(getLineIndexForPosition(adjustedY), activeOutput.value.length - 1),
        )
    }
    ...
    nextTick(() => {
        ...
        requestAnimationFrame(() => {
            ...
            const targetElement = Array.from(elements).find(
                (el) =>
                    el.textContent?.includes(targetLine) &&
                    !el.hasAttribute('data-pyro-terminal-separator'),
            )
...
watch(
    ...
    (newOutput, oldOutput) => {
        ...
        const shouldScroll =
            isScrolledToBottom.value || !userHasScrolled.value || isInitialLoad.value
