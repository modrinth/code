    ...
    >
        ...
        <div class="col-span-2 flex flex-col gap-1 sm:col-span-1">
            ...
            <div class="flex flex-wrap items-center gap-2 text-sm">
                ...
                <span v-if="(failedToCreate || failedToRestore) && (automated || backup.locked)"
                    >•</span
                >
                <span
            ...
            <template v-else>
                ...
                <ButtonStyled circular type="transparent">
                    ...
                    >
                        <MoreVerticalIcon />
                        <template #rename>
                            <EditIcon /> {{ formatMessage(messages.rename) }}
                        </template>
                        <template #restore>
                            <HistoryIcon /> {{ formatMessage(messages.restore) }}
                        </template>
                        <template v-if="backup.locked" #lock>
                            <LockOpenIcon /> {{ formatMessage(messages.unlock) }}
                        </template>
                        <template v-else #lock>
                            <LockIcon /> {{ formatMessage(messages.lock) }}
                        </template>
                        <template #delete>
