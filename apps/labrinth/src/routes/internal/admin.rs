...
pub async fn count_download(
	...
	let project_id: crate::database::models::ids::DBProjectId = download_body.project_id.into();

	let id_option = ariadne::ids::base62_impl::parse_base62(&download_body.version_name)
		.ok()
		.map(|x| x as i64);
...
pub async fn delphi_result_ingest(
	...
	let project =
		crate::database::models::DBProject::get_id(body.project_id.into(), &**pool, &redis)
			.await?
			.ok_or_else(|| {
				ApiError::InvalidInput(format!("Project {} does not exist", body.project_id))
			})?;
