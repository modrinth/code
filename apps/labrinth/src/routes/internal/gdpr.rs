...
pub async fn export(
	...
	let collection_ids = crate::database::models::DBUser::get_collections(user_id, &**pool).await?;
	let collections =
		crate::database::models::DBCollection::get_many(&collection_ids, &**pool, &redis)
			.await?
			.into_iter()
			.map(crate::models::collections::Collection::from)
			.collect::<Vec<_>>();

	let follows = crate::database::models::DBUser::get_follows(user_id, &**pool)
		.await?
		.into_iter()
		.map(crate::models::ids::ProjectId::from)
		.collect::<Vec<_>>();

	let projects = crate::database::models::DBUser::get_projects(user_id, &**pool, &redis)
		.await?
		.into_iter()
		.map(crate::models::ids::ProjectId::from)
		.collect::<Vec<_>>();

	let org_ids = crate::database::models::DBUser::get_organizations(user_id, &**pool).await?;
	let orgs = crate::database::models::organization_item::DBOrganization::get_many_ids(
		&org_ids, &**pool, &redis,
	)
	.await?
	.into_iter()
	...
	let pat_ids = crate::database::models::pat_item::DBPersonalAccessToken::get_user_pats(
		user_id, &**pool, &redis,
	)
	.await?;
	let pats = crate::database::models::pat_item::DBPersonalAccessToken::get_many_ids(
		&pat_ids, &**pool, &redis,
	)
	.await?
	.into_iter()
	...
	let payout_ids =
		crate::database::models::payout_item::DBPayout::get_all_for_user(user_id, &**pool).await?;

	let payouts = crate::database::models::payout_item::DBPayout::get_many(&payout_ids, &**pool)
		.await?
		.into_iter()
	...
	let report_ids =
		crate::database::models::user_item::DBUser::get_reports(user_id, &**pool).await?;
	let reports = crate::database::models::report_item::DBReport::get_many(&report_ids, &**pool)
		.await?
		.into_iter()
	...
	let messages =
		crate::database::models::thread_item::DBThreadMessage::get_many(&message_ids, &**pool)
			.await?
			.into_iter()
	...
	let uploaded_images = crate::database::models::image_item::DBImage::get_many(
		&uploaded_images_ids,
		&**pool,
		&redis,
	)
