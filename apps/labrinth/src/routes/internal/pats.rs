...
pub async fn get_pats(
	...
	let user = get_user_from_headers(&req, &**pool, &redis, &session_queue, Scopes::PAT_READ)
		.await?
		.1;

	let pat_ids = database::models::pat_item::DBPersonalAccessToken::get_user_pats(
		user.id.into(),
		&**pool,
		&redis,
	)
	.await?;
	let pats =
		database::models::pat_item::DBPersonalAccessToken::get_many_ids(&pat_ids, &**pool, &redis)
			.await?;
...
pub async fn create_pat(
	...
	let user = get_user_from_headers(&req, &**pool, &redis, &session_queue, Scopes::PAT_CREATE)
		.await?
		.1;
...
pub async fn edit_pat(
	...
	let user = get_user_from_headers(&req, &**pool, &redis, &session_queue, Scopes::PAT_WRITE)
		.await?
		.1;

	let id = id.into_inner().0;
	let pat = database::models::pat_item::DBPersonalAccessToken::get(&id, &**pool, &redis).await?;
...
pub async fn delete_pat(
	...
	let user = get_user_from_headers(&req, &**pool, &redis, &session_queue, Scopes::PAT_DELETE)
		.await?
		.1;
	let id = id.into_inner().0;
	let pat = database::models::pat_item::DBPersonalAccessToken::get(&id, &**pool, &redis).await?;
	...
	{
		let mut transaction = pool.begin().await?;
		database::models::pat_item::DBPersonalAccessToken::remove(pat.id, &mut transaction).await?;
		transaction.commit().await?;
		database::models::pat_item::DBPersonalAccessToken::clear_cache(
			vec![(Some(pat.id), Some(pat.access_token), Some(pat.user_id))],
			&redis,
