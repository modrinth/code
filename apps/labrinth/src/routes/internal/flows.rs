...
use crate::auth::validate::{get_full_user_from_headers, get_user_record_from_bearer_token};
...
impl TempUser {
	async fn create_account(
		...
		while username.is_none() {
			...
			let new_id =
				crate::database::models::DBUser::get(&test_username, client, redis).await?;
			...
			AuthProvider::Discord => {
				...
				TempUser {
					...
					avatar_url: discord_user
						.avatar
						.map(|x| format!("https://cdn.discordapp.com/avatars/{id}/{x}.webp")),
					bio: None,
					country: None,
				}
			}
...
pub async fn delete_auth_provider(
	...
	if user.auth_providers.is_none_or(|x| x.len() <= 1) && !user.has_password.unwrap_or(false) {
	...
	transaction.commit().await?;
	crate::database::models::DBUser::clear_caches(&[(user.id.into(), None)], &redis).await?;

	Ok(HttpResponse::NoContent().finish())
}

pub async fn check_sendy_subscription(email: &str) -> Result<bool, AuthenticationError> {
	...
	if url.is_empty() || url == "none" {
		tracing::info!("Sendy URL not set, returning false for subscription check");
		return Ok(false);
	}
...
pub async fn create_account_with_password(
	...
	if crate::database::models::DBUser::get(&new_account.username, &**pool, &redis)
		.await?
		.is_some()
	{
	...
	if !crate::database::models::DBUser::get_by_case_insensitive_email(&new_account.email, &**pool)
		.await?
		.is_empty()
	{
		return Err(ApiError::InvalidInput(
	...
	crate::database::models::DBUser {
		...
		badges: Badges::default(),
		allow_friend_requests: true,
		is_subscribed_to_newsletter: new_account.sign_up_newsletter.unwrap_or(false),
	}
...
pub async fn login_password(
	...
	let user = if let Some(user) =
		crate::database::models::DBUser::get(&login.username_or_email, &**pool, &redis).await?
	{
		user
	} else {
		let user = crate::database::models::DBUser::get_by_email(&login.username_or_email, &**pool)
			.await?
			.ok_or_else(|| AuthenticationError::InvalidCredentials)?;
...
async fn validate_2fa_code(
		...
		.map_err(|_| AuthenticationError::InvalidCredentials)?
	{
		...
		let backup_codes = crate::database::models::DBUser::get_backup_codes(user_id, pool).await?;
		...
		if !backup_codes.contains(&input) {
			...
			crate::database::models::DBUser::clear_caches(&[(user_id, None)], redis).await?;
...
pub async fn login_2fa(
	...
	if let DBFlow::Login2FA { user_id } = flow {
		let user = crate::database::models::DBUser::get_id(user_id, &**pool, &redis)
			.await?
			.ok_or_else(|| AuthenticationError::InvalidCredentials)?;
...
pub async fn remove_2fa(
	...
	transaction.commit().await?;
	crate::database::models::DBUser::clear_caches(&[(user.id, None)], &redis).await?;
...
pub async fn reset_password_begin(
	...
	let user = match crate::database::models::DBUser::get_by_case_insensitive_email(
	...
	.await?[..]
	{
		[] => {
			...
			crate::database::models::DBUser::get(&reset_password.username_or_email, &**pool, &redis)
				.await?
		}
		[user_id] => {
			...
			crate::database::models::DBUser::get_id(user_id, &**pool, &redis).await?
		}
		_ => {
			...
			if let Some(user_id) = crate::database::models::DBUser::get_by_email(
				&reset_password.username_or_email,
				&**pool,
			)
			.await?
...
pub async fn set_email(
	...
	if !crate::database::models::DBUser::get_by_case_insensitive_email(&email.email, &**pool)
		.await?
		.is_empty()
	{
		return Err(ApiError::InvalidInput(
...
pub async fn get_newsletter_subscription_status(
	...
	let user = get_full_user_from_headers(&req, &**pool, &redis, &session_queue, Scopes::USER_READ)
		.await?
		.1;
