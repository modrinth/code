...
use crate::models::billing::{
	Charge, ChargeStatus, ChargeType, PaymentPlatform, Price, PriceDuration, Product,
	ProductMetadata, ProductPrice, SubscriptionMetadata, SubscriptionStatus, UserSubscription,
};
...
use stripe::{
	...
	CustomerInvoiceSettings, CustomerPaymentMethodRetrieval, EventObject, EventType,
	PaymentIntentId, PaymentIntentOffSession, PaymentIntentSetupFutureUsage, PaymentMethodId,
...
pub async fn products(
	...
	let products = product_item::QueryProductWithPrices::list_purchaseable(&**pool, &redis).await?;
...
pub async fn subscriptions(
	...
	let subscriptions = user_subscription_item::DBUserSubscription::get_all_user(
		if let Some(user_id) = query.user_id {
			if user.role.is_admin() {
				user_id.into()
			} else {
				return Err(ApiError::InvalidInput(
					"You cannot see the subscriptions of other users!".to_string(),
				));
			}
		} else {
			user.id.into()
	...
	if let Some(charge) = DBCharge::get(id.into(), &**pool).await? {
		...
		let refunds = -refunds
			...
			.filter_map(|x| match x.status {
				ChargeStatus::Open | ChargeStatus::Processing | ChargeStatus::Succeeded => {
					Some(x.amount)
				}
				ChargeStatus::Failed | ChargeStatus::Cancelled | ChargeStatus::Expiring => None,
			})
			.sum::<i64>();
		...
		if (refundable - refund_amount) < 0 {
			return Err(ApiError::InvalidInput(
				"You cannot refund more than the amount of the charge!".to_string(),
			));
		}
		...
		let (id, net) = if refund_amount == 0 {
			...
			match charge.payment_platform {
				...
				PaymentPlatform::None => {
					return Err(ApiError::InvalidInput(
						"This charge was not processed via a payment platform.".to_owned(),
					));
				}
			}
		};
		...
		{
			let open_charge = DBCharge::get_open_subscription(subscription_id, &**pool).await?;
...
pub async fn edit_subscription(
	...
	if let Some(subscription) =
		user_subscription_item::DBUserSubscription::get(id.into(), &**pool).await?
	{
		...
		let current_price =
			product_item::DBProductPrice::get(subscription.price_id, &mut *transaction)
				.await?
				.ok_or_else(|| {
					ApiError::InvalidInput("Could not find current product price".to_string())
				})?;
		...
		if let Some(cancelled) = &edit_subscription.cancelled {
			...
			if !matches!(
				open_charge.status,
				ChargeStatus::Open | ChargeStatus::Cancelled | ChargeStatus::Failed
			) {
				return Err(ApiError::InvalidInput(
		...
		let intent = if let Some(product_id) = &edit_subscription.product {
			let product_price = product_item::DBProductPrice::get_all_product_prices(
				(*product_id).into(),
				&mut *transaction,
			)
			.await?
			...
			if open_charge.status == ChargeStatus::Expiring {
				let Some(new_region) = edit_subscription.region.as_ref().map(String::to_owned)
					...
					return Err(ApiError::InvalidInput(
						"You need to specify a region when promoting an expiring charge."
							.to_owned(),
					));
				};

				// We need a new interval when promoting the charge.
				let interval = edit_subscription.interval.ok_or_else(|| {
					ApiError::InvalidInput(
						"You need to specify an interval when promoting an expiring charge."
							.to_owned(),
					)
				})?;
				...
				let new_price_value = match product_price.prices {
					...
					Price::Recurring { ref intervals } => {
						*intervals.get(&interval).ok_or_else(|| {
							ApiError::InvalidInput(
								"Could not find a valid price for the specified duration"
									.to_owned(),
							)
						})?
					}
				...
				let mut intent = CreatePaymentIntent::new(new_price_value as i64, currency);
				...
				if let Some(payment_method) = &edit_subscription.payment_method {
					let Ok(payment_method_id) = PaymentMethodId::from_str(payment_method) else {
						return Err(ApiError::InvalidInput(
				...
				let intent = stripe::PaymentIntent::create(&stripe_client, intent).await?;
				...
				if current_amount > amount || proration < 30 {
					...
					if let Some(region) = &edit_subscription.region {
						metadata.insert("modrinth_new_region".to_string(), region.to_owned());
					}
					...
					if let Some(payment_method) = &edit_subscription.payment_method {
						let Ok(payment_method_id) = PaymentMethodId::from_str(payment_method)
						else {
							return Err(ApiError::InvalidInput(
								"Invalid payment method id".to_string(),
							));
...
pub async fn charges(
	...
	let charges = crate::database::models::charge_item::DBCharge::get_from_user(
		if let Some(user_id) = query.user_id {
			if user.role.is_admin() {
				user_id.into()
			} else {
...
pub async fn edit_payment_method(
	...
	if payment_method.customer.is_some_and(|x| x.id() == customer) || user.role.is_admin() {
...
pub async fn remove_payment_method(
	...
	let user_subscriptions =
		user_subscription_item::DBUserSubscription::get_all_user(user.id.into(), &**pool).await?;
	...
	let server_ids = servers
		...
		.filter_map(|x| {
			x.metadata.as_ref().and_then(|metadata| match metadata {
				SubscriptionMetadata::Pyro { id, region } => Some(ActiveServer {
					user_id: x.user_id.into(),
					server_id: id.clone(),
					price_id: x.price_id.into(),
					interval: x.interval,
...
pub async fn initiate_payment(
	...
	let (price, currency_code, interval, price_id, charge_id, charge_type) = match payment_request
		.charge
	{
		ChargeRequestType::Existing { id } => {
			let charge = crate::database::models::charge_item::DBCharge::get(id.into(), &**pool)
				.await?
				.ok_or_else(|| {
					ApiError::InvalidInput("Specified charge could not be found!".to_string())
				})?;
		...
		ChargeRequestType::New {
			...
			let mut product_prices =
				product_item::DBProductPrice::get_all_public_product_prices(product.id, &**pool)
					.await?;
			...
			if let Price::Recurring { .. } = price_item.prices
				&& product.unitary
			{
				let user_subscriptions = user_subscription_item::DBUserSubscription::get_all_user(
					user.id.into(),
					&**pool,
				)
				.await?;
	...
	if let Some(payment_intent_id) = &payment_request.existing_payment_intent {
		...
		if let Some(charge_id) = charge_id {
			metadata.insert("modrinth_charge_id".to_string(), to_base62(charge_id.0));
		} else {
			let mut transaction = pool.begin().await?;
			let charge_id = generate_charge_id(&mut transaction).await?;
			let subscription_id = generate_user_subscription_id(&mut transaction).await?;
...
pub async fn stripe_webhook(
	...
	if let Ok(event) = Webhook::construct_event(
		...
		struct PaymentIntentMetadata {
			...
			pub user_subscription_item: Option<user_subscription_item::DBUserSubscription>,
		...
		async fn get_payment_intent_metadata(
			...
			'metadata: {
				...
				let Some(user) =
					crate::database::models::user_item::DBUser::get_id(user_id, pool, redis)
						.await?
				else {
					break 'metadata;
				};
				...
				let new_region = metadata.get("modrinth_new_region").map(String::to_owned);

				let (charge, price, product, subscription, new_region) = if let Some(mut charge) =
					crate::database::models::charge_item::DBCharge::get(charge_id, pool).await?
				{
					let Some(price) =
						product_item::DBProductPrice::get(charge.price_id, pool).await?
					else {
						break 'metadata;
					};

					let Some(product) =
						product_item::DBProduct::get(price.product_id, pool).await?
					else {
						break 'metadata;
					};
					...
					charge.payment_platform_id = Some(payment_intent_id.to_string());
					charge.upsert(transaction).await?;

					if let Some(subscription_id) = charge.subscription_id {
						let Some(mut subscription) =
							user_subscription_item::DBUserSubscription::get(subscription_id, pool)
								.await?
						else {
							break 'metadata;
						};
						...
						(charge, price, product, Some(subscription), new_region)
					} else {
					...
					let Some(price_id) = metadata
						...
						.and_then(|x| parse_base62(x).ok())
						.map(|x| crate::database::models::ids::DBProductPriceId(x as i64))
					else {
						break 'metadata;
					};

					let Some(price) = product_item::DBProductPrice::get(price_id, pool).await?
					else {
						break 'metadata;
					};
					...
					let subscription = match &price.prices {
						...
						Price::Recurring { intervals } => {
							...
							if intervals.get(&interval).is_some() {
								...
								let subscription = if let Some(mut subscription) =
					...
					let charge = DBCharge {
						...
						subscription_interval: subscription.as_ref().map(|x| x.interval),
						payment_platform: PaymentPlatform::Stripe,
						payment_platform_id: Some(payment_intent_id.to_string()),
		...
		match event.type_ {
			EventType::PaymentIntentSucceeded => {
				if let EventObject::PaymentIntent(payment_intent) = event.data.object {
					...
					if let Some(latest_charge) = payment_intent.latest_charge {
						...
						if let Some(balance_transaction) =
							charge.balance_transaction.and_then(|x| x.into_object())
						{
							metadata.charge_item.net = Some(balance_transaction.net);
							metadata.charge_item.upsert(&mut transaction).await?;
						}
					}
					...
					match metadata.product_item.metadata {
						...
						ProductMetadata::Medal { .. } => {
							warn!("A payment processed for a free subscription");
						}

						ProductMetadata::Midas => {
							let badges = metadata.user_item.badges | Badges::MIDAS;
							...
							sqlx::query!(
                                ...
                                ",
								badges.bits() as i64,
								metadata.user_item.id as crate::database::models::ids::DBUserId,
							)
						...
						ProductMetadata::Pyro {
							...
							if let Some(ref mut subscription) = metadata.user_subscription_item {
								...
								if let Some(
									subscription_metadata @ (SubscriptionMetadata::Pyro {
										id,
										region: _,
									}
								...
								) = &subscription.metadata
								{
									...
									subscription.metadata = Some(SubscriptionMetadata::Pyro {
										id: id.to_string(),
										region,
									});
								} else {
									let (server_name, server_region, source) = if let Some(
									...
									{
										(server_name.clone(), server_region.clone(), source.clone())
									} else {
					...
					if let Some(mut subscription) = metadata.user_subscription_item {
						...
						if let Some(mut charge) = open_charge {
							if charge.status == ChargeStatus::Expiring {
								...
								charge.due = Utc::now() + subscription.interval.duration();
				...
				if let EventObject::PaymentIntent(payment_intent) = event.data.object {
					...
					if let Some(email) = metadata.user_item.email {
						let money = rusty_money::Money::from_minor(
							...
							rusty_money::iso::find(&metadata.charge_item.currency_code)
			...
			EventType::PaymentMethodAttached => {
				if let EventObject::PaymentMethod(payment_method) = event.data.object
					&& let Some(customer_id) = payment_method.customer.map(|x| x.id())
				{
					let customer =
...
async fn get_or_create_customer(
	...
	if let Some(customer_id) = stripe_customer_id.and_then(|x| stripe::CustomerId::from_str(x).ok())
	{
		...
		crate::database::models::user_item::DBUser::clear_caches(&[(user_id.into(), None)], redis)
			.await?;
...
pub async fn index_subscriptions(pool: PgPool, redis: RedisPool) {
	...
	let res = async {
		...
		let mut all_subscriptions = user_subscription_item::DBUserSubscription::get_many(
			&all_charges
				.iter()
		...
		for charge in all_charges {
			...
			let Some(user) = users.iter().find(|x| x.id == subscription.user_id) else {
				continue;
			};
			...
			let unprovisioned = match product.metadata {
				...
				ProductMetadata::Pyro { .. } | ProductMetadata::Medal { .. } => 'server: {
...
pub async fn try_process_user_redeemal(
	...
	let (price_duration, price_amount) = match medal_price.prices {
		...
		Price::Recurring { intervals } => {
			let Some((price_duration, price_amount)) = intervals.into_iter().next() else {
				return Err(ApiError::Conflict(
...
pub async fn index_billing(stripe_client: stripe::Client, pool: PgPool, redis: RedisPool) {
	...
	let res = async {
		...
		for mut charge in charges_to_do {
			let Some(product_price) = prices.iter().find(|x| x.id == charge.price_id) else {
				continue;
			};
			...
			if let Some(price) = price {
				...
				let Ok(currency) = Currency::from_str(&product_price.currency_code.to_lowercase())
				else {
					warn!(
