...
use crate::queue::socket::{ActiveSocket, ActiveSockets, SocketId, TunnelSocketType};
use crate::routes::ApiError;
use crate::sync::friends::{FRIENDS_CHANNEL_NAME, RedisFriendsMessage};
use crate::sync::status::{get_user_status, push_back_user_expiry, replace_user_status};
...
pub async fn ws_init(
	...
	let (scopes, db_user) =
		get_user_record_from_bearer_token(&req, Some(&auth.code), &**pool, &redis, &session_queue)
			.await?
	...
	let friends = DBFriend::get_user_friends(user.id.into(), Some(true), &**pool).await?;
	...
	broadcast_friends_message(&redis, RedisFriendsMessage::StatusUpdate { status }).await?;
	...
	actix_web::rt::spawn(async move {
		loop {
			...
			let futures_util::future::Either::Left((Some(msg), _)) = select(next, timeout).await
			else {
				break;
			};

			let message = match msg {
				Ok(Message::Text(text)) => ClientToServerMessage::deserialize(Either::Left(&text)),
			...
			match message {
				ClientToServerMessage::StatusUpdate { profile_name } => {
					if let Some(mut pair) = db.sockets.get_mut(&socket_id) {
						...
						if status.profile_name.as_ref().is_some_and(|x| x.len() > 64) {
							return;
						}
						...
						let _ = replace_user_status(Some(&old_status), Some(&user_status), &redis)
							.await;
				...
				ClientToServerMessage::SocketClose { socket } => {
					...
					if active_socket.owned_tunnel_sockets.remove(&socket).is_none() {
						continue;
					}
					let Some((_, tunnel_socket)) = db.tunnel_sockets.remove(&socket) else {
						continue;
					};
					match tunnel_socket.socket_type {
						TunnelSocketType::Listening => {
							let _ = broadcast_to_local_friends(
								user.id,
								ServerToClientMessage::FriendSocketStoppedListening {
									user: user.id,
								},
								&pool,
								&db,
						...
						TunnelSocketType::Connected { connected_to } => {
							let Some((_, other)) = db.tunnel_sockets.remove(&connected_to) else {
								continue;
							};
				...
				ClientToServerMessage::SocketSend { socket, data } => {
					let Some(tunnel_socket) = db.tunnel_sockets.get(&socket) else {
						continue;
					};
					if tunnel_socket.owner != socket_id {
						continue;
					}
					let TunnelSocketType::Connected { connected_to } = tunnel_socket.socket_type
					else {
						continue;
					};
					let Some(other_tunnel) = db.tunnel_sockets.get(&connected_to) else {
						continue;
					};
					let Some(other_user) = db.sockets.get(&other_tunnel.owner) else {
						continue;
					};
					let _ = send_message(
						&other_user,
...
async fn broadcast_to_known_local_friends(
	...
	for friend in friends {
		...
		if friend.accepted
			&& let Some(socket_ids) = sockets.sockets_by_user_id.get(&friend_id.into())
		{
...
pub async fn close_socket(
	...
	if let Some((_, socket)) = db.sockets.remove(&id) {
		...
		broadcast_friends_message(redis, RedisFriendsMessage::UserOffline { user: user_id })
			.await?;

		for owned_socket in socket.owned_tunnel_sockets {
			let Some((_, tunnel_socket)) = db.tunnel_sockets.remove(&owned_socket) else {
				continue;
			};
			match tunnel_socket.socket_type {
				TunnelSocketType::Listening => {
