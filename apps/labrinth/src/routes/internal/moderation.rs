...
pub async fn get_projects(
	...
	check_is_moderator_from_headers(&req, &**pool, &redis, &session_queue, Scopes::PROJECT_READ)
		.await?;
	...
	let projects: Vec<_> = database::DBProject::get_many_ids(&project_ids, &**pool, &redis)
		.await?
		.into_iter()
...
pub async fn get_project_meta(
	...
	let project = database::models::DBProject::get(&project_id, &**pool, &redis).await?;
