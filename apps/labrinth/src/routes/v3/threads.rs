...
pub async fn filter_authorized_threads(
	...
	let users: Vec<User> = database::models::DBUser::get_many_ids(&user_ids, &***pool, redis)
		.await?
		.into_iter()
		.map(From::from)
		.collect();
...
pub async fn thread_get(
	...
	let user = get_user_from_headers(&req, &**pool, &redis, &session_queue, Scopes::THREAD_READ)
		.await?
		.1;
		...
		&& is_authorized_thread(&data, &user, &pool).await?
	{
		...
		let users: Vec<User> = database::models::DBUser::get_many_ids(authors, &**pool, &redis)
			.await?
			.into_iter()
			.map(From::from)
			.collect();
...
pub async fn threads_get(
	...
	let threads_data = database::models::DBThread::get_many(&thread_ids, &**pool).await?;
...
pub async fn thread_send_message(
	...
	let user = get_user_from_headers(&req, &**pool, &redis, &session_queue, Scopes::THREAD_WRITE)
		.await?
		.1;
	...
	} = &new_message.body
	{
		...
		if let Some(replying_to) = replying_to {
			let thread_message =
				database::models::DBThreadMessage::get((*replying_to).into(), &**pool).await?;
	...
	if let Some(thread) = result {
		...
		if let Some(project_id) = thread.project_id {
			let project = database::models::DBProject::get_id(project_id, &**pool, &redis).await?;
			...
			let report = database::models::report_item::DBReport::get(report_id, &**pool).await?;
		...
		} = &new_message.body
		{
			for image_id in associated_images {
				if let Some(db_image) =
					image_item::DBImage::get((*image_id).into(), &mut *transaction, &redis).await?
				{
...
pub async fn message_delete(
	...
	let result =
		database::models::DBThreadMessage::get(info.into_inner().0.into(), &**pool).await?;
	...
	if let Some(thread) = result {
		...
		let images = database::DBImage::get_many_contexted(context, &mut transaction).await?;
		...
		for image in images {
			...
			database::DBImage::remove(image.id, &mut transaction, &redis).await?;
		}
		...
		database::models::DBThreadMessage::remove_full(thread.id, private, &mut transaction)
			.await?;
		transaction.commit().await?;
