...
pub async fn images_add(
	...
	match &mut context {
		ImageContext::Project { project_id } => {
			if let Some(id) = data.project_id {
				let project = project_item::DBProject::get(&id, &**pool, &redis).await?;
		...
		ImageContext::Version { version_id } => {
			if let Some(id) = data.version_id {
				let version = version_item::DBVersion::get(id.into(), &**pool, &redis).await?;
		...
		ImageContext::ThreadMessage { thread_message_id } => {
			if let Some(id) = data.thread_message_id {
				let thread_message = thread_item::DBThreadMessage::get(id.into(), &**pool)
					.await?
					.ok_or_else(|| {
	...
	let bytes =
		read_limited_from_payload(&mut payload, 1_048_576, "Icons must be smaller than 1MiB")
			.await?;
