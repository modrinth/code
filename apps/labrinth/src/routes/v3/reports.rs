...
pub async fn report_create(
	...
	let current_user =
		get_user_from_headers(&req, &**pool, &redis, &session_queue, Scopes::REPORT_CREATE)
			.await?
			.1;
...
pub async fn reports(
	...
	let user = get_user_from_headers(&req, &**pool, &redis, &session_queue, Scopes::REPORT_READ)
		.await?
		.1;
	...
	let query_reports =
		crate::database::models::report_item::DBReport::get_many(&report_ids, &**pool).await?;
...
pub async fn report_get(
	...
	let report = crate::database::models::report_item::DBReport::get(id, &**pool).await?;
...
pub async fn report_edit(
	...
	let user = get_user_from_headers(&req, &**pool, &redis, &session_queue, Scopes::REPORT_WRITE)
		.await?
		.1;
	let id = info.into_inner().0.into();
...
pub async fn report_delete(
	...
	check_is_moderator_from_headers(&req, &**pool, &redis, &session_queue, Scopes::REPORT_DELETE)
		.await?;
	...
	let result =
		crate::database::models::report_item::DBReport::remove_full(id.into(), &mut transaction)
			.await?;
	transaction.commit().await?;
