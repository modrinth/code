...
use crate::{
	auth::{filter_visible_collections, filter_visible_projects, get_user_from_headers},
...
pub async fn admin_user_email(
	...
	if !user.role.is_admin() {
		return Err(ApiError::CustomAuthentication(
			"You do not have permission to get a user from their email!".to_string(),
		));
	}
	...
	.map(|x| x.id)
	.ok_or_else(|| {
		ApiError::InvalidInput("The email provided is not associated with a user!".to_string())
	})?;

	let user = DBUser::get_id(crate::database::models::DBUserId(user_id), &**pool, &redis).await?;
...
pub async fn projects_list(
	...
	if let Some(id) = id_option.map(|x| x.id) {
		...
		let projects: Vec<_> =
			crate::database::DBProject::get_many_ids(&project_data, &**pool, &redis).await?;
...
pub async fn user_auth_get(
	...
	let (scopes, mut user) =
		get_user_from_headers(&req, &**pool, &redis, &session_queue, Scopes::USER_READ).await?;
...
pub async fn user_get(
	...
	if let Some(data) = user_data {
		...
		let response: crate::models::users::User = if auth_user.is_some_and(|x| x.role.is_admin()) {
...
pub async fn collections_list(
	...
	if let Some(id) = id_option.map(|x| x.id) {
		...
		let response: Vec<_> =
			crate::database::models::DBCollection::get_many(&collection_data, &**pool, &redis)
				.await?;

		let collections = filter_visible_collections(response, &user, true).await?;
...
pub async fn orgs_list(
	...
	if let Some(id) = id_option.map(|x| x.id) {
		...
		let teams_data = crate::database::models::DBTeamMember::get_from_team_full_many(
			&team_ids, &**pool, &redis,
		)
		.await?;
...
pub async fn user_edit(
	...
	let (scopes, user) =
		get_user_from_headers(&req, &**pool, &redis, &session_queue, Scopes::USER_WRITE).await?;
	...
	if let Some(actual_user) = id_option {
		...
		if user.id == user_id || user.role.is_mod() {
			...
			if let Some(username) = &new_user.username {
				let existing_user_id_option = DBUser::get(username, &**pool, &redis).await?;
			...
			DBUser::clear_caches(&[(id, Some(actual_user.username))], &redis).await?;
...
pub async fn user_delete(
	...
	let user = get_user_from_headers(&req, &**pool, &redis, &session_queue, Scopes::USER_DELETE)
		.await?
		.1;
	let id_option = DBUser::get(&info.into_inner().0, &**pool, &redis).await?;
...
pub async fn user_follows(
	...
	let user = get_user_from_headers(&req, &**pool, &redis, &session_queue, Scopes::USER_READ)
		.await?
		.1;
	let id_option = DBUser::get(&info.into_inner().0, &**pool, &redis).await?;
