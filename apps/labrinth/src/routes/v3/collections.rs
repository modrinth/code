...
pub async fn collections_get(
	...
	let ids = ids
		.into_iter()
		.map(|x| parse_base62(x).map(|x| database::models::DBCollectionId(x as i64)))
		.collect::<Result<Vec<_>, _>>()?;

	let collections_data = database::models::DBCollection::get_many(&ids, &**pool, &redis).await?;
	...
	let collections = filter_visible_collections(collections_data, &user_option, false).await?;
...
pub async fn collection_get(
	...
	let collection_data = database::models::DBCollection::get(id, &**pool, &redis).await?;
	let user_option = get_user_from_headers(
		&req,
		&**pool,
		&redis,
...
pub async fn collection_edit(
	...
	let string = info.into_inner().0;
	let id = database::models::DBCollectionId(parse_base62(&string)? as i64);
	let result = database::models::DBCollection::get(id, &**pool, &redis).await?;
	...
	if let Some(collection_item) = result {
		...
		if let Some(new_project_ids) = &new_collection.new_projects {
			...
			for project_id in new_project_ids {
				let project = database::models::DBProject::get(project_id, &**pool, &redis)
					.await?
					.ok_or_else(|| {
		...
		transaction.commit().await?;
		database::models::DBCollection::clear_cache(collection_item.id, &redis).await?;
...
pub async fn collection_icon_edit(
	...
	let string = info.into_inner().0;
	let id = database::models::DBCollectionId(parse_base62(&string)? as i64);
	let collection_item = database::models::DBCollection::get(id, &**pool, &redis)
		.await?
		.ok_or_else(|| {
...
pub async fn collection_delete(
	...
	let result =
		database::models::DBCollection::remove(collection.id, &mut transaction, &redis).await?;
