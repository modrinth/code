...
use crate::models::projects::{
	License, Link, MonetizationStatus, ProjectStatus, SideTypesMigrationReviewStatus, VersionStatus,
};
...
async fn project_create_inner(
	...
	let current_user =
		get_user_from_headers(&req, pool, redis, session_queue, Scopes::PROJECT_CREATE)
			.await?
			.1;
	...
	let mut gallery_urls = Vec::new();
	{
		...
		if let Some(slug_project_id) = slug_project_id_option {
			let slug_project_id: models::ids::DBProjectId = slug_project_id.into();
...

	{
		...
		if let Some(organization_id) = project_create_data.organization_id {
			let org = models::DBOrganization::get_id(organization_id.into(), pool, redis)
				.await?
				.ok_or_else(|| {
			...
			let team_member =
				models::DBTeamMember::get_from_user_id(org.team_id, current_user.id.into(), pool)
					.await?;

			let perms =
				OrganizationPermissions::get_permissions_by_role(&current_user.role, &team_member);

			if !perms.is_some_and(|x| x.contains(OrganizationPermissions::ADD_PROJECT)) {
