...
use crate::models::projects::{
	MonetizationStatus, Project, ProjectStatus, SearchRequest, SideTypesMigrationReviewStatus,
};
...
pub async fn random_projects_get(
	...
	let projects_data = db_models::DBProject::get_many_ids(&project_ids, &**pool, &redis)
		.await?
		.into_iter()
		.map(Project::from)
		.collect::<Vec<_>>();
...
pub async fn projects_get(
	...
	let projects_data = db_models::DBProject::get_many(&ids, &**pool, &redis).await?;

	let user_option =
		get_user_from_headers(&req, &**pool, &redis, &session_queue, Scopes::PROJECT_READ)
			.await
			.map(|x| x.1)
			.ok();
...
pub async fn project_get(
	...
	let project_data = db_models::DBProject::get(&string, &**pool, &redis).await?;
	let user_option =
		get_user_from_headers(&req, &**pool, &redis, &session_queue, Scopes::PROJECT_READ)
			.await
			.map(|x| x.1)
...
pub struct EditProject {
	...
	pub side_types_migration_review_status: Option<SideTypesMigrationReviewStatus>,
	#[serde(flatten)]
...
pub async fn project_edit(
	...
	let user = get_user_from_headers(&req, &**pool, &redis, &session_queue, Scopes::PROJECT_WRITE)
		.await?
		.1;
	...
	let Some(project_item) =
		db_models::DBProject::get(&info.into_inner().0, &**pool, &redis).await?
	else {
	...
	if let Some(name) = &new_project.name {
		if !perms.contains(ProjectPermissions::EDIT_DETAILS) {
			return Err(ApiError::CustomAuthentication(
				"You do not have the permissions to edit the name of this project!".to_string(),
			));
		}
	...
	if let Some(summary) = &new_project.summary {
		if !perms.contains(ProjectPermissions::EDIT_DETAILS) {
			return Err(ApiError::CustomAuthentication(
				"You do not have the permissions to edit the summary of this project!".to_string(),
			));
		}
	...
	if let Some(status) = &new_project.status {
		if !perms.contains(ProjectPermissions::EDIT_DETAILS) {
			return Err(ApiError::CustomAuthentication(
				"You do not have the permissions to edit the status of this project!".to_string(),
			));
		}

		if !(user.role.is_mod()
			|| !project_item.inner.status.is_approved() && status == &ProjectStatus::Processing
	...
	if let Some(slug) = &new_project.slug {
		if !perms.contains(ProjectPermissions::EDIT_DETAILS) {
			return Err(ApiError::CustomAuthentication(
				"You do not have the permissions to edit the slug of this project!".to_string(),
			));
		}
	...
	if let Some(license) = &new_project.license_id {
		if !perms.contains(ProjectPermissions::EDIT_DETAILS) {
			return Err(ApiError::CustomAuthentication(
				"You do not have the permissions to edit the license of this project!".to_string(),
			));
		}
		...
		&& !links.is_empty()
	{
		if !perms.contains(ProjectPermissions::EDIT_DETAILS) {
			return Err(ApiError::CustomAuthentication(
				"You do not have the permissions to edit the links of this project!".to_string(),
			));
		}
		...
		for (platform, url) in links {
			if let Some(url) = url {
				let platform_id =
					db_models::categories::LinkPlatform::get_id(platform, &mut *transaction)
						.await?
						.ok_or_else(|| {
	...
	if let Some(moderation_message_body) = &new_project.moderation_message_body {
		if !user.role.is_mod()
			&& (!project_item.inner.status.is_approved() || moderation_message_body.is_some())
		{
			return Err(ApiError::CustomAuthentication(
	...
	if let Some(monetization_status) = &new_project.monetization_status {
		...
		if (*monetization_status == MonetizationStatus::ForceDemonetized
			|| project_item.inner.monetization_status == MonetizationStatus::ForceDemonetized)
			&& !user.role.is_mod()
		{
			return Err(ApiError::CustomAuthentication(
				"You do not have the permissions to edit the monetization status of this project!"
	...
	if !new_project.loader_fields.is_empty() {
		for version in
			db_models::DBVersion::get_many(&project_item.versions, &**pool, &redis).await?
		{
	...
	db_models::DBProject::clear_cache(project_item.inner.id, project_item.inner.slug, None, &redis)
		.await?;
...
pub async fn project_get_check(
	...
	let project_data = db_models::DBProject::get(&slug, &**pool, &redis).await?;
...
pub async fn dependency_list(
	...
	if let Some(project) = result {
		...
		let dependencies =
			database::DBProject::get_dependencies(project.inner.id, &**pool, &redis).await?;
...
pub async fn projects_edit(
	...
	let project_ids: Vec<db_ids::DBProjectId> = serde_json::from_str::<Vec<ProjectId>>(&ids.ids)?
		.into_iter()
		.map(|x| x.into())
		.collect();

	let projects_data = db_models::DBProject::get_many_ids(&project_ids, &**pool, &redis).await?;
	...
	let team_members =
		db_models::DBTeamMember::get_from_team_full_many(&team_ids, &**pool, &redis).await?;
	...
	let organizations =
		db_models::DBOrganization::get_many_ids(&organization_ids, &**pool, &redis).await?;
	...
	let organization_team_members =
		db_models::DBTeamMember::get_from_team_full_many(&organization_team_ids, &**pool, &redis)
			.await?;
	...
	for project in projects_data {
		...
		bulk_edit_project_categories(
			...
			CategoryChanges {
				...
				remove_categories: &bulk_edit_project.remove_additional_categories,
			},
			256,
			true,
			&mut transaction,
		...
		db_models::DBProject::clear_cache(project.inner.id, project.inner.slug, None, &redis)
			.await?;
	}
...
pub async fn project_icon_edit(
	...
	let bytes =
		read_limited_from_payload(&mut payload, 262144, "Icons must be smaller than 256KiB")
			.await?;
...
pub async fn add_gallery_item(
	...
	DBGalleryItem::insert_many(gallery_item, project_item.inner.id, &mut transaction).await?;
...
pub async fn delete_gallery_item(
	...
	let project_item =
		db_models::DBProject::get_id(database::models::DBProjectId(item.mod_id), &**pool, &redis)
			.await?
			.ok_or_else(|| {
				ApiError::InvalidInput("The specified project does not exist!".to_string())
			})?;
...
pub async fn project_delete(
	...
	let uploaded_images = db_models::DBImage::get_many_contexted(context, &mut transaction).await?;
	...
	let result = db_models::DBProject::remove(project.inner.id, &mut transaction, &redis).await?;
...
pub async fn project_follow(
	...
	let user = get_user_from_headers(&req, &**pool, &redis, &session_queue, Scopes::USER_WRITE)
		.await?
		.1;
	let string = info.into_inner().0;
...
pub async fn project_get_organization(
	...
	if !is_visible_project(&result.inner, &current_user, &pool, false).await? {
		...
		let organization = db_models::DBOrganization::get_id(organization_id, &**pool, &redis)
			.await?
			.ok_or_else(|| {
		...
		let members_data =
			DBTeamMember::get_from_team_full(organization.team_id, &**pool, &redis).await?;
		...
		let team_members: Vec<_> = members_data
			...
			.filter(|x| {
				logged_in
					|| x.accepted || user_id
					.is_some_and(|y: crate::database::models::DBUserId| y == x.user_id)
			})
			.filter_map(|data| {
