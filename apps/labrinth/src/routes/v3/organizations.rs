...
use crate::database::models::{DBOrganization, generate_organization_id, team_item};
...
pub async fn organization_projects_get(
	...
	if let Some(organization) = organization_data {
		...
		let projects_data =
			crate::database::models::DBProject::get_many_ids(&project_ids, &**pool, &redis).await?;
...
pub async fn organization_create(
	...
	let results =
		DBOrganization::get_many(&organization_strings, &mut *transaction, &redis).await?;
	if !results.is_empty() {
	...
	let member_data = DBTeamMember::get_from_team_full(team_id, &**pool, &redis)
		.await?
		.into_iter()
		.next();
...
pub async fn organization_get(
	...
	if let Some(data) = organization_data {
		let members_data = DBTeamMember::get_from_team_full(data.team_id, &**pool, &redis).await?;
...
pub async fn organizations_get(
	...
	let organizations_data = DBOrganization::get_many(&ids, &**pool, &redis).await?;
...
pub async fn organizations_edit(
	...
	let result = database::models::DBOrganization::get(&string, &**pool, &redis).await?;
...
pub async fn organization_delete(
	...
	let organization = database::models::DBOrganization::get(&string, &**pool, &redis)
		.await?
		.ok_or_else(|| {
	...
	if !user.role.is_admin() {
		let team_member = database::models::DBTeamMember::get_from_user_id_organization(
			organization.id,
			user.id.into(),
			false,
			&**pool,
	...
	let result =
		database::models::DBOrganization::remove(organization.id, &mut transaction, &redis).await?;

	transaction.commit().await?;

	database::models::DBOrganization::clear_cache(organization.id, Some(organization.slug), &redis)
		.await?;
	...
	if !organization_project_teams.is_empty() {
		database::models::DBUser::clear_project_cache(&[owner_id], &redis).await?;
	}
...
pub async fn organization_projects_add(
	...
	let organization = database::models::DBOrganization::get(&info, &**pool, &redis)
		.await?
		.ok_or_else(|| {
			ApiError::InvalidInput("The specified organization does not exist!".to_string())
		})?;

	let project_item = database::models::DBProject::get(&project_info.project_id, &**pool, &redis)
		.await?
		.ok_or_else(|| {
	...
	let project_team_member = database::models::DBTeamMember::get_from_user_id_project(
	...
	let organization_team_member = database::models::DBTeamMember::get_from_user_id_organization(
		organization.id,
		current_user.id.into(),
		false,
		&**pool,
	...
	if permissions.contains(OrganizationPermissions::ADD_PROJECT) {
		...
		database::models::DBUser::clear_project_cache(&[current_user.id.into()], &redis).await?;
		database::models::DBTeamMember::clear_cache(project_item.inner.team_id, &redis).await?;
...
pub async fn organization_projects_remove(
	...
	let organization = database::models::DBOrganization::get(&organization_id, &**pool, &redis)
		.await?
		.ok_or_else(|| {
			ApiError::InvalidInput("The specified organization does not exist!".to_string())
		})?;

	let project_item = database::models::DBProject::get(&project_id, &**pool, &redis)
		.await?
		.ok_or_else(|| {
			ApiError::InvalidInput("The specified project does not exist!".to_string())
		})?;
	...
	if permissions.contains(OrganizationPermissions::REMOVE_PROJECT) {
		...
		let new_owner = database::models::DBTeamMember::get_from_user_id_project(
			project_item.inner.id,
			data.new_owner.into(),
			true,
			&**pool,
...
pub async fn organization_icon_edit(
	...
	let organization_item = database::models::DBOrganization::get(&string, &**pool, &redis)
		.await?
		.ok_or_else(|| {
			ApiError::InvalidInput("The specified organization does not exist!".to_string())
		})?;
