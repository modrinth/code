...
pub async fn team_members_get_project(
	...
	let project_data = crate::database::models::DBProject::get(&string, &**pool, &redis).await?;
	...
	if let Some(project) = project_data {
		...
		let members_data =
			DBTeamMember::get_from_team_full(project.inner.team_id, &**pool, &redis).await?;
		...
		let logged_in = if let Some(user_id) = user_id {
			let (team_member, organization_team_member) =
				DBTeamMember::get_for_project_permissions(&project.inner, user_id, &**pool).await?;
...
pub async fn team_members_get_organization(
	...
	let organization_data =
		crate::database::models::DBOrganization::get(&string, &**pool, &redis).await?;
...
pub async fn team_members_get(
	...
	let members_data = DBTeamMember::get_from_team_full(id.into(), &**pool, &redis).await?;
	let users = crate::database::models::DBUser::get_many_ids(
		&members_data.iter().map(|x| x.user_id).collect::<Vec<_>>(),
		&**pool,
		&redis,
	...
	let team_members: Vec<_> = members_data
		...
		.filter(|x| {
			logged_in
				|| x.accepted
				|| user_id.is_some_and(|y: crate::database::models::DBUserId| y == x.user_id)
		})
		.filter_map(|data| {
			users
				.iter()
...
pub async fn teams_get(
	...
	let teams_data = DBTeamMember::get_from_team_full_many(&team_ids, &**pool, &redis).await?;
	let users = crate::database::models::DBUser::get_many_ids(
...
pub async fn join_team(
	...
	let current_user =
		get_user_from_headers(&req, &**pool, &redis, &session_queue, Scopes::PROJECT_WRITE)
			.await?
			.1;

	let member =
		DBTeamMember::get_from_user_id_pending(team_id, current_user.id.into(), &**pool).await?;
...
pub async fn add_team_member(
	...
	let member = DBTeamMember::get_from_user_id(team_id, current_user.id.into(), &**pool).await?;
	match team_association {
		...
		TeamAssociationId::Project(pid) => {
			let organization =
				DBOrganization::get_associated_organization_project_id(pid, &**pool).await?;
	...
	let request =
		DBTeamMember::get_from_user_id_pending(team_id, new_member.user_id.into(), &**pool).await?;
	...
	let new_user =
		crate::database::models::DBUser::get_id(new_member.user_id.into(), &**pool, &redis)
			.await?
	...
	if let TeamAssociationId::Project(pid) = team_association {
		...
		let new_user_organization_team_member = if let Some(organization) = &organization {
			DBTeamMember::get_from_user_id(organization.team_id, new_user.id, &**pool).await?
		} else {
			None
		};
...
pub async fn edit_team_member(
	...
	let team_association = DBTeam::get_association(id, &**pool)
		.await?
		.ok_or_else(|| ApiError::InvalidInput("The team specified does not exist".to_string()))?;
	let member = DBTeamMember::get_from_user_id(id, current_user.id.into(), &**pool).await?;
	let edit_member_db = DBTeamMember::get_from_user_id_pending(id, user_id, &**pool)
		.await?
		.ok_or_else(|| {
	...
	match team_association {
		TeamAssociationId::Project(project_id) => {
			let organization =
				DBOrganization::get_associated_organization_project_id(project_id, &**pool).await?;
			let organization_team_member = if let Some(organization) = &organization {
				DBTeamMember::get_from_user_id(
					organization.team_id,
					current_user.id.into(),
	...
	if let Some(payouts_split) = edit_member.payouts_split
		&& (payouts_split < Decimal::ZERO || payouts_split > Decimal::from(5000))
	{
		return Err(ApiError::InvalidInput(
			"Payouts split must be between 0 and 5000!".to_string(),
		));
...
pub async fn transfer_ownership(
	...
	let team_association_id = DBTeam::get_association(id.into(), &**pool).await?;
	...
	if !current_user.role.is_admin() {
		let member = DBTeamMember::get_from_user_id(id.into(), current_user.id.into(), &**pool)
			.await?
			.ok_or_else(|| {
				ApiError::CustomAuthentication(
					"You don't have permission to edit members of this team".to_string(),
	...
	let new_member = DBTeamMember::get_from_user_id(id.into(), new_owner.user_id.into(), &**pool)
		.await?
		.ok_or_else(|| {
	...
	if let Some(former_owner) = DBTeamMember::get_from_team_full(id.into(), &**pool, &redis)
		.await?
		.into_iter()
		.find(|x| x.is_owner)
	{
	...
	let project_teams_edited =
		if let Some(TeamAssociationId::Organization(oid)) = team_association_id {
			...
			let team_ids: Vec<crate::database::models::ids::DBTeamId> = team_ids
				.into_iter()
			...
			for team_id in &team_ids {
				DBTeamMember::delete(*team_id, new_owner.user_id.into(), &mut transaction).await?;
			}
...
pub async fn remove_team_member(
	...
	let delete_member = DBTeamMember::get_from_user_id_pending(id, user_id, &**pool).await?;
