...
pub async fn playtimes_get(
	...
	let user = get_user_from_headers(&req, &**pool, &redis, &session_queue, Scopes::ANALYTICS)
		.await
		.map(|x| x.1)?;
...
pub async fn downloads_get(
	...
	let user_option =
		get_user_from_headers(&req, &**pool, &redis, &session_queue, Scopes::ANALYTICS)
			.await
			.map(|x| x.1)?;
...
async fn filter_allowed_ids(
	...
	let project_ids = if let Some(project_strings) = project_ids {
		let projects_data =
			database::models::DBProject::get_many(&project_strings, &***pool, redis).await?;
		...
		let team_members =
			database::models::DBTeamMember::get_from_team_full_many(&team_ids, &***pool, redis)
				.await?;
		...
		let organizations =
			database::models::DBOrganization::get_many_ids(&organization_ids, &***pool, redis)
				.await?;
		...
		let organization_team_ids = organizations
			...
			.map(|x| x.team_id)
			.collect::<Vec<database::models::DBTeamId>>();
		let organization_team_members = database::models::DBTeamMember::get_from_team_full_many(
