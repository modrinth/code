...
pub async fn get_version_from_hash(
	...
	let algorithm = hash_query
		.algorithm
		.clone()
		.unwrap_or_else(|| default_algorithm_from_hashes(std::slice::from_ref(&hash)));
	...
	if let Some(file) = file {
		let version = database::models::DBVersion::get(file.version_id, &**pool, &redis).await?;
...
pub async fn get_update_from_hash(
	...
	.await? && let Some(project) =
		database::models::DBProject::get_id(file.project_id, &**pool, &redis).await?
	{
		let mut versions =
...
pub async fn get_projects_from_hashes(
	...
	let projects_data = filter_visible_projects(
		database::models::DBProject::get_many_ids(&project_ids, &**pool, &redis).await?,
		&user_option,
		&pool,
		false,
	)
...
pub async fn update_individual_files(
	...
	for project in projects {
		for file in files.iter().filter(|x| x.project_id == project.inner.id) {
			if let Some(hash) = file.hashes.get(&algorithm)
				&& let Some(query_file) = update_data.hashes.iter().find(|x| &x.hash == hash)
			{
...
pub async fn delete_file(
	...
	if let Some(row) = file {
		if !user.role.is_admin() {
			let team_member = database::models::DBTeamMember::get_from_user_id_version(
				row.version_id,
				user.id.into(),
				&**pool,
			)
		...
		let version = database::models::DBVersion::get(row.version_id, &**pool, &redis).await?;
		if let Some(version) = version {
