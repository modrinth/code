		...
		| "PAYMENT.PAYOUTS-ITEM.CANCELED" => {
			...
			if let Some(result) = result {
				...
				crate::database::models::user_item::DBUser::clear_caches(
					&[(crate::database::models::DBUserId(result.user_id), None)],
					&redis,
				)
				.await?;
			}
...
pub async fn user_payouts(
	...
	let user = get_user_from_headers(&req, &**pool, &redis, &session_queue, Scopes::PAYOUTS_READ)
		.await?
		.1;

	let payout_ids =
		crate::database::models::payout_item::DBPayout::get_all_for_user(user.id.into(), &**pool)
			.await?;
	let payouts =
		crate::database::models::payout_item::DBPayout::get_many(&payout_ids, &**pool).await?;
		...
		PayoutMethodType::Venmo | PayoutMethodType::PayPal => {
			...
			if let Some(link) = res.links.first() {
				...
				if let Ok(res) = payouts_queue
					...
					.await && let Some(data) = res.items.first()
				{
...
pub async fn cancel_payout(
	...
	let user = get_user_from_headers(&req, &**pool, &redis, &session_queue, Scopes::PAYOUTS_WRITE)
		.await?
		.1;

	let id = info.into_inner().0;
	let payout = crate::database::models::payout_item::DBPayout::get(id.into(), &**pool).await?;
				...
				match method {
					PayoutMethodType::Venmo | PayoutMethodType::PayPal => {
						payouts
							.make_paypal_request::<(), ()>(
								Method::POST,
								&format!("payments/payouts-item/{platform_id}/cancel"),
								None,
								None,
								None,
							)
...
async fn get_user_balance(
	...
	let (withdrawn, fees) = withdrawn.map_or((Decimal::ZERO, Decimal::ZERO), |x| {
		(
...
pub async fn platform_revenue(
	...
	.map(|x| RevenueData {
		...
		revenue: x.sum.unwrap_or(Decimal::ZERO) * Decimal::from(25) / Decimal::from(75),
