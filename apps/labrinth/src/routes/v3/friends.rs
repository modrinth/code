...
use crate::routes::internal::statuses::{broadcast_friends_message, send_message_to_user};
...
pub async fn add_friend(
	...
	let friend = crate::database::models::DBUser::get(&string, &**pool, &redis).await?;

	if let Some(friend) = friend {
		let mut transaction = pool.begin().await?;

		if let Some(friend) = crate::database::models::friend_item::DBFriend::get_friend(
			user.id.into(),
			friend.id,
			&**pool,
		)
		.await?
		{
			...
			async fn send_friend_status(
				...
				if let Some(friend_status) = get_user_status(user_id.into(), sockets, redis).await {
			...
			send_friend_status(friend.user_id, friend.friend_id, &db, &redis).await?;
			send_friend_status(friend.friend_id, friend.user_id, &db, &redis).await?;
		} else {
...
pub async fn friends(
	...
	let friends = crate::database::models::friend_item::DBFriend::get_user_friends(
		user.id.into(),
		None,
		&**pool,
	)
