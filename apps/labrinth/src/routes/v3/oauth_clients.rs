...
pub async fn get_user_clients(
	...
	if let Some(target_user) = target_user {
		...
		let clients = DBOAuthClient::get_all_user_clients(target_user.id, &**pool).await?;
...
pub async fn oauth_client_delete(
	...
	let client = DBOAuthClient::get(client_id.into_inner().into(), &**pool).await?;
...
pub async fn get_user_oauth_authorizations(
	...
	let authorizations =
		DBOAuthClientAuthorization::get_all_for_user(current_user.id.into(), &**pool).await?;
...
pub async fn revoke_oauth_authorization(
	...
	DBOAuthClientAuthorization::remove(info.client_id.into(), current_user.id.into(), &**pool)
		.await?;
...
async fn edit_redirects(
	...
	.await?;
	DBOAuthClient::insert_redirect_uris(&redirects_to_add, &mut **transaction).await?;
