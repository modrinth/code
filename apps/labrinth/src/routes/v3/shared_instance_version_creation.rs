...
use crate::database::models::{
	DBSharedInstanceId, DBSharedInstanceVersionId, generate_shared_instance_version_id,
};
...
async fn shared_instance_version_create_inner(
	...
	if !user.role.is_mod() && instance.owner_id != user.id.into() {
		let permissions =
			DBSharedInstanceUser::get_user_permissions(instance_id, user.id.into(), pool).await?;
		if let Some(permissions) = permissions {
			if !permissions.contains(SharedInstanceUserPermissions::UPLOAD_VERSION) {
	...
	let version_id = generate_shared_instance_version_id(&mut *transaction).await?;
	...
	while let Some(chunk) = payload.next().await {
		...
		if file_data.len() + chunk.len() <= MAX_FILE_SIZE {
			...
			file_data.extend_from_slice(&chunk[..MAX_FILE_SIZE - file_data.len()]);
			break;
		}
	}
