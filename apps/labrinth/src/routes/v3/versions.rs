...
use crate::database::models::version_item::{DBLoaderVersion, DependencyBuilder};
...
pub async fn version_project_get_helper(
	...
	let result = database::models::DBProject::get(&id.0, &**pool, &redis).await?;
	...
	if let Some(project) = result {
		...
		let versions =
			database::models::DBVersion::get_many(&project.versions, &**pool, &redis).await?;
		...
		if let Some(version) = version
			&& is_visible_version(&version.inner, &user_option, &pool, &redis).await?
		{
...
pub async fn versions_get(
	...
	let versions_data =
		database::models::DBVersion::get_many(&version_ids, &**pool, &redis).await?;

	let user_option =
		get_user_from_headers(&req, &**pool, &redis, &session_queue, Scopes::VERSION_READ)
			.await
			.map(|x| x.1)
			.ok();
...
pub async fn version_get_helper(
	...
	let version_data = database::models::DBVersion::get(id.into(), &**pool, &redis).await?;
...
pub async fn version_edit_helper(
	...
	let user = get_user_from_headers(&req, &**pool, &redis, &session_queue, Scopes::VERSION_WRITE)
		.await?
		.1;
	...
	let result = database::models::DBVersion::get(version_id, &**pool, &redis).await?;

	if let Some(version_item) = result {
		let team_member = database::models::DBTeamMember::get_from_user_id_project(
		...
		if let Some(perms) = permissions {
			...
			if let Some(loaders) = &new_version.loaders {
				...
				DBLoaderVersion::insert_many(loader_versions, &mut transaction).await?;
			...
			if let Some(downloads) = &new_version.downloads {
				...
				sqlx::query!(
                    ...
                    ",
					diff as i32,
					version_item.inner.project_id as database::models::ids::DBProjectId,
				)
				.execute(&mut *transaction)
				.await?;
			}
			...
			database::models::DBVersion::clear_cache(&version_item, &redis).await?;
...
pub async fn version_list(
	...
	let result = database::models::DBProject::get(&string, &**pool, &redis).await?;
...
pub async fn version_delete(
	...
	let uploaded_images =
		database::models::DBImage::get_many_contexted(context, &mut transaction).await?;
	for image in uploaded_images {
		image_item::DBImage::remove(image.id, &mut transaction, &redis).await?;
	}

	let result =
		database::models::DBVersion::remove_full(version.inner.id, &redis, &mut transaction)
			.await?;
	transaction.commit().await?;

	database::models::DBProject::clear_cache(version.inner.project_id, None, Some(true), &redis)
		.await?;
