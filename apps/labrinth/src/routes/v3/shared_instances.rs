...
pub async fn shared_instance_create(
	...
	let new_instance: CreateSharedInstance = read_typed_from_payload(&mut body).await?;
...
pub async fn shared_instance_list(
	...
	let instances = DBSharedInstance::list_for_user(user.id.into(), &**pool).await?;
	let instances = try_join_all(instances.into_iter().map(
		async |instance| -> Result<SharedInstance, ApiError> {
			...
			Ok(SharedInstance::from_db(
				instance,
				Some(DBSharedInstanceUser::get_from_instance(instance_id, &**pool, &redis).await?),
				version,
			))
		},
	))
...
pub async fn shared_instance_get(
	...
	if let Some(shared_instance) = shared_instance {
		let users = DBSharedInstanceUser::get_from_instance(id, &**pool, &redis).await?;

		let privately_accessible =
			user.is_some_and(|user| can_access_instance_privately(&shared_instance, &users, &user));
		if !shared_instance.public && !privately_accessible {
			return Err(ApiError::NotFound);
		}

		let current_version = if let Some(version_id) = shared_instance.current_version_id {
			DBSharedInstanceVersion::get(version_id, &**pool).await?
		} else {
			None
		};
...
pub async fn shared_instance_edit(
	...
	let id = info.into_inner().0.into();
	let edit_instance: EditSharedInstance = read_typed_from_payload(&mut body).await?;
	...
	if !user.role.is_mod() && instance.owner_id != user.id.into() {
		let permissions =
			DBSharedInstanceUser::get_user_permissions(id, user.id.into(), &**pool).await?;
		if let Some(permissions) = permissions {
			if !permissions.contains(SharedInstanceUserPermissions::EDIT) {
				return Err(ApiError::CustomAuthentication(
					"You do not have permission to edit this shared instance.".to_string(),
				));
			}
		} else {
			return Err(ApiError::NotFound);
...
pub async fn shared_instance_version_list(
	...
	if let Some(shared_instance) = shared_instance {
		if !can_access_instance_as_maybe_user(&pool, &redis, &shared_instance, user).await? {
			return Err(ApiError::NotFound);
		}

		let versions = DBSharedInstanceVersion::get_for_instance(id, &**pool).await?;
...
pub async fn shared_instance_version_get(
	...
	if let Some(version) = version {
		let instance = DBSharedInstance::get(version.shared_instance_id, &**pool).await?;
		if let Some(instance) = instance {
			if !can_access_instance_as_maybe_user(&pool, &redis, &instance, user).await? {
				return Err(ApiError::NotFound);
			}
...
async fn can_access_instance_as_maybe_user(
	...
	let users = DBSharedInstanceUser::get_from_instance(instance.id, pool, redis).await?;
	Ok(user.is_some_and(|user| can_access_instance_privately(instance, &users, &user)))
}
...
pub async fn shared_instance_version_delete(
	...
	let shared_instance_version = DBSharedInstanceVersion::get(version_id, &**pool).await?;

	if let Some(shared_instance_version) = shared_instance_version {
		let shared_instance =
			DBSharedInstance::get(shared_instance_version.shared_instance_id, &**pool).await?;
		if let Some(shared_instance) = shared_instance {
			...
			delete_instance_version(shared_instance.id, version_id, &pool).await?;
...
pub async fn shared_instance_version_download(
	...
	if let Some(version) = version {
		...
		if let Some(instance) = instance {
			...
			let url = file_host.get_url_for_private_file(&file_name, 180).await?;
