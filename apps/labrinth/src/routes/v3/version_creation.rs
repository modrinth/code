...
use crate::models::projects::{
	Dependency, FileType, Loader, Version, VersionFile, VersionStatus, VersionType,
};
...
async fn version_create_inner(
	...
	let user = get_user_from_headers(&req, pool, redis, session_queue, Scopes::VERSION_CREATE)
		.await?
		.1;
...
async fn upload_file_to_version_inner(
	...
	if models::DBProject::get_id(version.inner.project_id, &mut **transaction, &redis)
		.await?
		.is_none()
	{
		return Err(CreateError::InvalidInput(
