...
pub fn jemalloc_memory_stats(registry: &Registry) -> Result<(), prometheus::Error> {
	let allocated_mem = IntGauge::new("labrinth_memory_allocated", "labrinth allocated memory")?;
	let resident_mem = IntGauge::new("labrinth_resident_memory", "labrinth resident memory")?;
