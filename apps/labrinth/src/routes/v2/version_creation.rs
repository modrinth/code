...
///
/// There must be at least one file attached to each new version, unless the status is `draft`.
/// `.mrpack`, `.jar`, `.zip`, and `.litemod` files are accepted.
///
...
/// You can name the file parts anything you would like, but you must list each of the parts' names
/// in `file_parts`, and optionally, provide one to use as the primary file in `primary_file`.
///
