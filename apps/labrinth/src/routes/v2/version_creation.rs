...
use crate::models::projects::{Dependency, FileType, Loader, Version, VersionStatus, VersionType};
					...
					.any(|field| field == "environment")
				{
					...
					fields.insert(
						...
						get_example_version_fields(legacy_create.project_id, client, &redis)
							...
							.map_or(json!("unknown"), |f| f.value.serialize_internal()),
					);
				}
...
async fn get_example_version_fields(
	...
	let Some(vid) = project_item::DBProject::get_id(project_id.into(), &**pool, redis)
		.await?
	...
	let Some(example_version) = version_item::DBVersion::get(vid, &**pool, redis).await? else {
		return Ok(None);
	};
