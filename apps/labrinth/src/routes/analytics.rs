...
pub async fn page_view_ingest(
	...
	let user = get_user_from_headers(&req, &**pool, &redis, &session_queue, Scopes::empty())
		.await
		.ok();
	...
	let ip =
		crate::util::ip::convert_to_ip_v6(if let Some(header) = headers.get("cf-connecting-ip") {
			header
		} else {
	...
	if let Some(segments) = url.path_segments() {
		...
		if segments_vec.len() >= 2 {
			...
			if PROJECT_TYPES.contains(&segments_vec[0]) {
				let project =
					crate::database::models::DBProject::get(segments_vec[1], &**pool, &redis)
						.await?;
