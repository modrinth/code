...
impl RedisPool {
	...
	pub async fn register_and_set_metrics(
		...
		let redis_max_size =
			IntGauge::new("labrinth_redis_pool_max_size", "Maximum size of Redis pool")?;
		let redis_size = IntGauge::new("labrinth_redis_pool_size", "Current size of Redis pool")?;
		...
		S: Display + Clone + DeserializeOwned + Serialize + Debug,
	{
		...
		if !subscribe_ids.is_empty() {
			fetch_tasks.push(Either::Right(async {
				...
				let (return_values, _) = get_cached_values(subscribe_ids).await?;
