use crate::database::models::{DBSharedInstanceId, DBSharedInstanceVersionId, DBUserId};
...
impl From<SharedInstanceQueryResult> for DBSharedInstance {
	fn from(val: SharedInstanceQueryResult) -> Self {
		DBSharedInstance {
			...
			public: val.public,
			current_version_id: val.current_version_id.map(DBSharedInstanceVersionId),
		}
	}
}
				...
				async |user_ids| {
					...
					.try_fold(DashMap::new(), |acc: DashMap<_, Vec<_>>, m| {
						acc.entry(m.shared_instance_id)
							.or_default()
							.push(DBSharedInstanceUser {
								user_id: DBUserId(m.user_id),
								shared_instance_id: DBSharedInstanceId(m.shared_instance_id),
								permissions: SharedInstanceUserPermissions::from_bits(
