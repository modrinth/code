use crate::database::models::{DBProductId, DBProductPriceId, DatabaseError, product_item};
...
impl DBProduct {
	...
	pub async fn get_by_type<'a, E>(exec: E, r#type: &str) -> Result<Vec<Self>, DatabaseError>
	where
		E: sqlx::PgExecutor<'a>,
	{
		let maybe_row = select_products_with_predicate!("WHERE metadata ->> 'type' = $1", r#type)
			.fetch_all(exec)
			.await?;
...
impl QueryProductWithPrices {
		...
		E: sqlx::Executor<'a, Database = sqlx::Postgres> + Copy,
	{
		...
		let prices = product_item::DBProductPrice::get_all_public_products_prices(
...
impl DBProductPrice {
	...
	pub async fn get_all_public_product_prices(
		...
		let res = Self::get_all_public_products_prices(&[product_id], exec).await?;
	...
	pub async fn get_all_public_products_prices(
		...
		Self::get_all_products_prices_with_visibility(product_ids, Some(true), exec).await
	}
	...
	pub async fn get_all_products_prices(
		...
		Self::get_all_products_prices_with_visibility(product_ids, None, exec).await
	}
	...
	async fn get_all_products_prices_with_visibility(
		...
		let predicate = |acc: DashMap<DBProductId, Vec<DBProductPrice>>, x| {
			if let Ok(item) = <ProductPriceQueryResult as TryInto<DBProductPrice>>::try_into(x) {
		...
		let prices = match public_filter {
			None => {
				select_prices_with_predicate!("WHERE product_id = ANY($1::bigint[])", ids_ref,)
					.fetch(exec)
