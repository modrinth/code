...
impl UserRedeemal {
	pub async fn get_pending<'a, E>(exec: E, limit: i64) -> sqlx::Result<Vec<UserRedeemal>>
	where
		E: sqlx::PgExecutor<'a>,
	{
		let redeemals = query!(
	...
	pub async fn update_status_if_pending<'a, E>(&self, exec: E) -> sqlx::Result<bool>
	where
		E: sqlx::PgExecutor<'a>,
	{
		let query = query!(
...
impl RedeemalLookupFields {
		...
		E: sqlx::PgExecutor<'a>,
	{
		...
		Ok(maybe_row.map(|row| RedeemalLookupFields {
			...
			redeemal_status: row.status.as_deref().map(Status::from_str_or_default),
		}))
	}
}
