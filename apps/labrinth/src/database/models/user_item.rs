...
impl DBUser {
	...
	pub async fn get_by_email<'a, E>(email: &str, exec: E) -> Result<Option<DBUserId>, sqlx::Error>
	where
	...
	pub async fn remove(
		...
		if let Some(delete_user) = user {
			DBUser::clear_caches(&[(id, Some(delete_user.username))], redis).await?;

			let deleted_user: DBUserId = crate::models::users::DELETED_USER.into();
			...
			for collection_id in user_collections {
				models::DBCollection::remove(collection_id, transaction, redis).await?;
			}
			...
			for x in open_subscriptions {
				let charge = DBCharge::get_open_subscription(x.id, &mut **transaction).await?;
