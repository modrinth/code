...
impl VersionField {
		...
		for item in items {
			...
			match item.value {
				...
				VersionFieldValue::Boolean(b) => {
					query_version_fields.push(base.clone().with_int_value(b as i32))
				}
	...
	pub fn from_query_json(
		...
		query_loader_fields
			.iter()
			.flat_map(|q| {
				let Some(loader_field_type) =
					LoaderFieldType::build(&q.field_type, q.enum_type.map(|l| l.0))
				else {
					return vec![];
				};
				let loader_field = LoaderField {
...
impl VersionFieldValue {
	...
	pub fn build_many(
		...
		let mut value = match field_type {
			...
			LoaderFieldType::Integer => qvfs
				...
				.collect::<Result<Vec<(DBVersionId, VersionFieldValue)>, DatabaseError>>()?,
	...
	pub fn contains_json_value(&self, value: &serde_json::Value) -> bool {
		match self {
			...
			VersionFieldValue::ArrayBoolean(v) => value.as_bool().is_some_and(|b| v.contains(&b)),
