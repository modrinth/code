use crate::file_hosting::{
	DeleteFileData, FileHost, FileHostPublicity, FileHostingError, UploadFileData,
};
...
impl S3Host {
	pub fn new(
		...
		let create_bucket = |config: S3BucketConfig| -> Result<_, FileHostingError> {
			...
			.map_err(|e| FileHostingError::S3Error("creating Bucket instance", e))?;
...
impl FileHost for S3Host {
	...
	async fn get_url_for_private_file(
		...
		let url = self
			...
			.await
			.map_err(|e| FileHostingError::S3Error("generating presigned URL", e))?;
		Ok(url)
	}
