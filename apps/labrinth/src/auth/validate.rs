	...
	E: sqlx::Executor<'a, Database = sqlx::Postgres> + Copy,
{
	...
	let Some((scopes, db_user)) =
		get_user_record_from_bearer_token(req, None, executor, redis, session_queue).await?
	else {
		return Ok(None);
	};
...
{
	let (scopes, db_user) =
		get_full_user_from_headers(req, executor, redis, session_queue, required_scopes).await?;
	...
	E: sqlx::Executor<'a, Database = sqlx::Postgres> + Copy,
{
	...
	let possible_user = match token.split_once('_') {
		Some(("mrp", _)) => {
			let pat = crate::database::models::pat_item::DBPersonalAccessToken::get(
			...
			let user = user_item::DBUser::get_id(pat.user_id, executor, redis).await?;
		...
		Some(("mra", _)) => {
			let session =
				crate::database::models::session_item::DBSession::get(token, executor, redis)
					.await?
					.ok_or_else(|| AuthenticationError::InvalidCredentials)?;
			...
			let user = user_item::DBUser::get_id(session.user_id, executor, redis).await?;
		...
		Some(("mro", _)) => {
			...
			let user = user_item::DBUser::get_id(access_token.user_id, executor, redis).await?;
