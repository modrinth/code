...
pub fn send_email_raw(to: String, subject: String, body: String) -> Result<(), MailError> {
	...
	let creds = (!username.is_empty()).then(|| Credentials::new(username, password));
	let tls_setting = match dotenvy::var("SMTP_TLS")?.as_str() {
		"none" => Tls::None,
		"opportunistic_start_tls" => Tls::Opportunistic(TlsParameters::new(host.to_string())?),
		"requires_start_tls" => Tls::Required(TlsParameters::new(host.to_string())?),
