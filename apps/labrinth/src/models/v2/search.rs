...
impl LegacyResultSearchProject {
	pub fn from(result_search_project: ResultSearchProject) -> Self {
		...
		let environment = get_one_string_loader_field("environment").unwrap_or("unknown");

		let (client_side, server_side) = v2_reroute::convert_v3_environment_to_v2_side_types(
			environment,
