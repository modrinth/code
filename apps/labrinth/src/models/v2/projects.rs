...
impl LegacyProject {
	...
	pub fn from(data: Project, versions_item: Option<version_item::VersionQueryResult>) -> Self {
		...
		if let Some(versions_item) = versions_item {
			...
			let fields = versions_item
				...
				.collect::<HashMap<_, _>>();
			(client_side, server_side) = v2_reroute::convert_v3_side_types_to_v2_side_types(
				&fields,
		...
		E: sqlx::Acquire<'a, Database = sqlx::Postgres>,
	{
		let version_ids: Vec<_> = data
			...
			.collect();
		let example_versions = version_item::DBVersion::get_many(&version_ids, exec, redis).await?;
