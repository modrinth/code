...
use crate::database::models::{
	DBProjectId, DBVersionId, LoaderFieldEnumId, LoaderFieldEnumValueId, LoaderFieldId,
};
	...
	for project in db_projects {
		...
		if let Some(versions) = versions.remove(&project.id) {
			...
			for version in versions {
				...
				let (client_side, server_side) = v2_reroute::convert_v3_side_types_to_v2_side_types(
...
async fn index_versions(
	...
	let mut res_versions: HashMap<DBProjectId, Vec<PartialVersion>> = HashMap::new();
