...
pub fn app_setup(
	...
	if enable_background_tasks {
		...
		let local_index_interval =
			Duration::from_secs(parse_var("LOCAL_INDEX_INTERVAL").unwrap_or(3600));
		let pool_ref = pool.clone();
		...
		scheduler.run(local_index_interval, move || {
			...
			async move {
				background_task::index_search(pool_ref, redis_pool_ref, search_config_ref).await;
			}
		});
		...
		let version_index_interval =
			Duration::from_secs(parse_var("VERSION_INDEX_INTERVAL").unwrap_or(1800));
		let pool_ref = pool.clone();
		let redis_pool_ref = redis_pool.clone();
		...
		actix_rt::spawn(async move {
			loop {
				routes::internal::billing::index_subscriptions(pool_ref.clone(), redis_ref.clone())
					.await;
				tokio::time::sleep(Duration::from_secs(60 * 5)).await;
			}
		});
	...
	let ip_salt = Pepper {
		pepper: ariadne::ids::Base62Id(ariadne::ids::random_base62(11)).to_string(),
	};
	...
	scheduler.run(Duration::from_secs(60 * 60 * 6), move || {
		...
		async move {
			...
			let result = insert_bank_balances(&payouts_queue_ref, &pool_ref).await;
			if let Err(e) = result {
	...
	match storage_backend.as_deref() {
		Some("s3") => {
			let mut check_var_set = |var_prefix| {
				failed |= check_var::<String>(&format!("S3_{var_prefix}_BUCKET_NAME"));
				failed |= check_var::<bool>(&format!("S3_{var_prefix}_USES_PATH_STYLE_BUCKET"));
				failed |= check_var::<String>(&format!("S3_{var_prefix}_REGION"));
				failed |= check_var::<String>(&format!("S3_{var_prefix}_URL"));
				failed |= check_var::<String>(&format!("S3_{var_prefix}_ACCESS_TOKEN"));
				failed |= check_var::<String>(&format!("S3_{var_prefix}_SECRET"));
			};
