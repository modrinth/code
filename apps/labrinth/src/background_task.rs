...
impl BackgroundTask {
	pub async fn run(
		...
		match self {
			...
			IndexBilling => {
				crate::routes::internal::billing::index_billing(stripe_client, pool, redis_pool)
					.await
			}
			IndexSubscriptions => {
				crate::routes::internal::billing::index_subscriptions(pool, redis_pool).await
			}
		}
	}
}
...
pub async fn update_versions(pool: sqlx::Pool<Postgres>, redis_pool: RedisPool) {
...
pub async fn payouts(pool: sqlx::Pool<Postgres>, clickhouse: clickhouse::Client) {
