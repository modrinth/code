use crate::validate::{
	MaybeProtectedZipFile, PLAUSIBLE_PACK_REGEX, SupportedGameVersions, ValidationError,
	ValidationResult,
};
use chrono::DateTime;
...
impl super::Validator for DataPackValidator {
	...
	fn get_supported_game_versions(&self) -> SupportedGameVersions {
		...
		SupportedGameVersions::PastDate(DateTime::from_timestamp(1508889600, 0).unwrap())
	}
	...
	fn validate_maybe_protected_zip(
		...
		if match file {
			MaybeProtectedZipFile::Unprotected(archive) => archive.by_name("pack.mcmeta").is_ok(),
