...
pub trait Validator: Sync {
	...
	fn validate_maybe_protected_zip(
		...
		match file {
			MaybeProtectedZipFile::Unprotected(archive) => self.validate(archive),
...
static PLAUSIBLE_PACK_REGEX: LazyLock<regex::bytes::Regex> = LazyLock::new(|| {
...
async fn validate_minecraft_file(
	...
	actix_web::web::block(move || {
		let mut zip = match ZipArchive::new(Cursor::new(Bytes::clone(&data))) {
			...
			Err(read_error) => MaybeProtectedZipFile::MaybeProtected { read_error, data },
		};
