...
static LINK_OR_IP: LazyLock<Regex> = LazyLock::new(|| {
	Regex::new(concat!(
		r"(?i)(?:https?://|www\.)[^\s<>()]+|",
...
pub(super) fn is_likely_english(text: &str) -> bool {
	let words = WORD.find_iter(text).count();
	if words < 8 || text.chars().count() < 35 {
		return true;
	}

	let mut scripts = HashMap::<&'static str, usize>::new();
	for character in text.chars().filter(|character| character.is_alphabetic())
	{
		let code = character as u32;
		let script = if code <= 0x024f {
			"latin"
		} else if is_in_ranges(code, &[(0x0400, 0x052f)]) {
			"cyrillic"
		} else if is_in_ranges(code, &[(0x0370, 0x03ff)]) {
			"greek"
		} else if is_in_ranges(
			code,
			&[
				(0x3040, 0x30ff),
				(0x3400, 0x4dbf),
	...
	let total = scripts.values().sum::<usize>();
	total == 0
		|| scripts.get("latin").copied().unwrap_or_default() * 10 >= total * 7
}
