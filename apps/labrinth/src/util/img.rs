...
use image::{DynamicImage, EncodableLayout, GenericImageView, ImageError, ImageFormat};
use sha1::Digest;
use std::io::Cursor;
use webp::Encoder;
...
pub async fn upload_image_optimized(
	...
	Ok(UploadImageResult {
		url: processed_upload_data
			.clone()
			.map_or_else(|| url.clone(), |x| format!("{}/{}", cdn_url, x.file_name)),
...
pub async fn delete_unused_images(
	...
	let uploaded_images =
		database::models::DBImage::get_many_contexted(context, transaction).await?;
