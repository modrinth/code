...
impl AsyncRateLimiter {
	...
	pub async fn check_rate_limit(&self, key: &str) -> RateLimitDecision {
		...
		let remaining_capacity = ((max_tat_delta - (new_tat - now)) / increment).max(0) as u32;
...
pub async fn rate_limit_middleware(
	...
	if let Some(key) = req.headers().get("x-ratelimit-key")
		&& key.to_str().ok() == dotenvy::var("RATE_LIMIT_IGNORE_KEY").ok().as_deref()
	{
	...
	if let Some(ip) = ip {
		...
		if decision.allowed {
			...
			headers.insert(
				actix_web::http::header::HeaderName::from_str("x-ratelimit-remaining").unwrap(),
			...
			headers.insert(
				actix_web::http::header::HeaderName::from_str("Access-Control-Allow-Origin")
					.unwrap(),
				"*".parse().unwrap(),
			);
