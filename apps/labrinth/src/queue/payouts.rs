...
impl PayoutsQueue {
	...
	pub async fn make_paypal_request<T: Serialize, X: DeserializeOwned>(
		...
		if !status.is_success() {
			...
			if let Ok(mut error) = serde_json::from_value::<PayPalError>(value.clone()) {
	...
	pub async fn get_brex_balance() -> Result<Option<AccountBalance>, ApiError> {
		...
		Ok(Some(AccountBalance {
			...
			pending: Decimal::from(
				res.items
					.iter()
					.map(|x| x.current_balance.amount - x.available_balance.amount)
					.sum::<i64>(),
			) / Decimal::from(100),
		}))
	}
	...
	pub async fn get_tremendous_balance(&self) -> Result<Option<AccountBalance>, ApiError> {
		#[derive(Deserialize)]
		...
		Ok(val
			...
			.map(|x| AccountBalance {
				available: Decimal::from(x.meta.available_cents) / Decimal::from(100),
				pending: Decimal::from(x.meta.pending_cents) / Decimal::from(100),
			}))
	}
}
...
pub async fn insert_bank_balances(payouts: &PayoutsQueue, pool: &PgPool) -> Result<(), ApiError> {
	let mut transaction = pool.begin().await?;
	...
	let mut add_balance = |account_type: &str, balance: Option<AccountBalance>| {
