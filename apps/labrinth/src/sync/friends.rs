...
use crate::routes::internal::statuses::{broadcast_to_local_friends, send_message_to_user};
...
pub async fn handle_pubsub(mut pubsub: PubSub, pool: PgPool, sockets: Data<ActiveSockets>) {
	...
	while let Some(message) = stream.next().await {
		...
		actix_rt::spawn(async move {
			match payload {
				...
				Ok(RedisFriendsMessage::DirectStatusUpdate { to_user, status }) => {
