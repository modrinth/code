...
use common::environment::{TestEnvironment, with_test_environment, with_test_environment_all};
...
async fn test_add_remove_project() {
	...
	with_test_environment(None, |test_env: TestEnvironment<ApiV3>| async move {
		...
		let hash = sha1::Sha1::digest(basic_mod_file.bytes()).encode_hex::<String>();
		let version = api
