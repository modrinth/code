...
// Экспорт middleware
// Plugin / in-game token APIs retired — product is site + launcher only.
const pluginGone = (_req, res) => {
    res.status(410).json({
        error: 'gone',
        message: 'Plugin and in-game token APIs are retired. Use the Owyx launcher and site account.'
    });
};
router.post('/generate-game-token', pluginGone);
router.post('/verify-game-token', pluginGone);
router.get('/game-tokens', pluginGone);
router.post('/create-game-session', pluginGone);
router.post('/check-game-session', pluginGone);
