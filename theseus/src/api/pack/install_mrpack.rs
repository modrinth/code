...
use crate::pack::install_from::{
    set_profile_information, EnvType, PackFile, PackFileHash, PackDependency,
};
use crate::prelude::{ProfilePathId, ModLoader};
use crate::state::{SideType, ProfileInstallStage, LinkedData, Profiles};
...
use super::install_from::{
    ...
    CreatePackLocation, PackFormat, CreatePackDescription,
};
...
pub async fn install_pack(
    ...
    let result = install_pack_files(create_pack).await;
    ...
    if let Some(zip_index) = zip_index_option {
        ...
        loading_try_for_each_concurrent(
            ...
            |project| {
                ...
                async move {
                    ...
                    let existing_file = profile_path.get_full_path().await?.join(&project.path);
        ...
        for index in 0..zip_reader.file().entries().len() {
                ...
                && !file.filename().ends_with('/')
            {
                ...
                // Remove this file if a corresponding one exists in the filesystem
                let existing_file = profile_path.get_full_path().await?.join(&new_path);
                if existing_file.exists() {
                    io::remove_file(&existing_file).await?;
                }
            }
