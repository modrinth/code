...
use crate::{event::LoadingBarId, pack::install_from::CreatePackDescription, prelude::ModLoader, state::ProfileInstallStage, State};

use super::{copy_dir_to, copy_dotminecraft};
...
pub async fn import_curseforge(    curseforge_instance_folder: PathBuf, // instance's folder
    ...
    let minecraft_instance: String =
        fs::read_to_string(&curseforge_instance_folder.join("minecraftinstance.json"))
            .await?;
    let minecraft_instance: MinecraftInstance = serde_json::from_str::<MinecraftInstance>(&minecraft_instance)?;
    let override_title: Option<String> = minecraft_instance.name.clone();
    let backup_name = format!("Curseforge-{}", curseforge_instance_folder.file_name().map(|a| a.to_string_lossy().to_string()).unwrap_or("Unknown".to_string()));
    ...
    if curseforge_instance_folder.join("manifest.json").exists() {
            // Load manifest.json
        let cf_manifest: String =
        fs::read_to_string(&curseforge_instance_folder.join("manifest.json"))
            .await?;
...
crate::api::profile::edit(&profile_path, |prof| {
    prof.metadata.name = override_title
        ...
        .unwrap_or_else(|| backup_name.to_string());
    prof.metadata.icon = icon.clone();
    prof.metadata.game_version =  minecraft_instance.game_version.clone();
